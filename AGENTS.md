# KRUTH ELEMENT (Platform E) — Project Brief for Codex
...
- **ตรวจสอบแล้วว่าไฟล์ต้นฉบับไม่มีข้อมูลนี้เลย** (`detailed_profile`/`challenges_shadow`
  เป็นเรื่องพลังงานตัวเลข ไม่ใช่ประวัติบุคคล) — ข้อมูลนี้ Codex ค้นคว้า+เขียนขึ้นใหม่ทั้งหมด
...
## 9. ลำดับงานที่แนะนำสำหรับ Codex
...
- **dev server:** ใช้ `.Codex/launch.json` (ชี้ node binary ตรงๆ เพราะ npm shebang หา node ไม่เจอ)
...
|---|---|
| Router | Codex Haiku 4.5 → Gemini 3.5 Flash → Codex Sonnet 5 |
| AI-1 นักค้นคว้า | Codex Sonnet 5 (web search) → Gemini 3.5 Flash → Codex Opus 4.8 |
| AI-2 อาจารย์ลาลา | **OpenAI gpt-5.5** → Gemini 3.5 Flash → Codex Opus 4.8 → template non-LLM |
...
**สลับ web search กลับเป็นรุ่นพื้นฐาน** (`lib/ai/Codex.ts`) — วัด A/B ด้วยฝันประโยคเดียวกัน
...
- ชั้น 3 ใช้ Codex Haiku 4.5 · วัดจริง **in 1,073 / out 81 = ฿0.053/ครั้ง** (~฿53/1,000 ครั้ง)
...
```
lib/ai/{index,types,Codex,openai,gemini}.ts   ชั้น AI + fallback chain
...
```
อ่าน AGENTS.md §15 บล็อก "✅ face-card เฟส 1" + "✅ โหมดความรักและเนื้อคู่ v1" + คิวเปิดงาน 10 ส.ค. 2569 แล้วรัน health check (tsc + test + build ควรได้ 483) · รายงานสถานะ · แล้วทำต่อข้อ 3 (เอกสารทางเลือกตาราง 9 กลุ่มอักษร→ธาตุ) หรือทวงงานฝั่งผู้ใช้ข้อ 4
```
...
- `app/api/face-card/route.ts`: consent → ตรวจไฟล์ (magic bytes ≤2MB) → ล็อกอิน+บัญชีถาวร →
  **ฟรี 1 ครั้ง/บัญชี (นับแถวจริง ถาวร) → 40 เครดิต** → ตรวจหน้า (Codex vision — infra พังไม่บล็อก)
...
- **ทางแก้ที่ทำเลย: prompt caching ของ planner ใช้ได้แล้ว** — ธง §10 เดิม ("อย่าลอง") หมดอายุ
  ...
  (Codex.ts ห่อ system เป็น block + cache_control) · `calcCost` คิด cache read 0.1×/write 1.25× ·
...
- **role AI ใหม่ "memory"** — Codex เท่านั้น (Haiku→Sonnet) เหตุผลเดียวกับ vision: ประวัติดวง
...
- **ชื่อที่ผู้ใช้เห็น = "LaLa Lucky Chat"** (h1 หน้าแรก · title แท็บ · login · LINE welcome ·
  ...
  (AGENTS.md, DB, เอกสารต้นทาง, prompt ภายในของ router/planner/admin ไม่เปลี่ยน)
...
3.5 **✅ วิเคราะห์ธาตุจากรูปภาพ เฟส 1 — เสร็จ 30 ก.ค. 2569 (ตกลงหลักการกับผู้ใช้แล้ว):**
   ...
   ใช้ vision ของ Gemini/Codex ผ่าน `lib/ai/` เดิมได้ fallback+cost log ฟรี)
   ...
   - **ชั้น AI:** role `"vision"` ใหม่ใน `lib/ai/` — **Codex เท่านั้น** (Haiku 4.5 → Sonnet 5)
...
```
1. planner  Codex Haiku (role "router")  → JSON แผน   → parsePlannerJson → interpretPlannerOutput
...
```
1. Codex Haiku  → คืน JSON "แผน"           ~฿0.05
...
| ต้นทุน | ฿0.6-1.2 | **~฿0.35** |
| provider | Codex เป็นหลัก | **ใช้ได้ทั้ง 3 เจ้า** |
...
**บทบาท AI (ผู้ใช้เลือกเอง):** Codex = คนเลือก · **OpenAI gpt-5.5 = คนเล่า** (สีสันดีกว่า)
...
```
อ่าน AGENTS.md §16 แล้วต่อเฟส 2 ของ AI Chat แบบยืดหยุ่น:
ต่อ AI จริงเข้ากับ lib/chat/plan.ts ที่มีอยู่แล้ว (Codex Haiku วางแผน → validateChatPlan
