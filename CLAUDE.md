...
## 0.0 🔴 กติกาการทำงานกับผู้ใช้ (ผู้ใช้สั่ง 9 ก.ย. 2569 — บังคับทุกเซสชัน)

- **สิ่งที่ดีอยู่แล้วและไม่มีคำสั่งให้แก้ไข/ปรับปรุง → ไม่ต้องไปแก้** (ห้ามรีแฟกเตอร์/ปรับ UX/เปลี่ยน default พ่วง
  ไปกับงานอื่นโดยไม่ได้สั่ง)
- **เห็นว่าควรแก้ → แจ้งเหตุผลสั้นๆ แล้วรออนุมัติก่อน** ค่อยลงมือ (ยกเว้นบั๊กที่ทำให้งานที่สั่งไว้พังหรือเทสต์/build
  ไม่ผ่าน — แก้ได้แต่ต้องรายงาน)
- แก้เฉพาะขอบเขตที่สั่ง แล้วรายงานตรงๆ ว่าแตะอะไร/ไม่แตะอะไร
...
1. **พลังงานส่วนบุคคล 00-99 = สูตรรวม Birth+Name+Time+Day ทุกโหมด (ผู้ใช้เคาะใหม่ 31 ส.ค. 2569)**
   เดิมข้อนี้เคยล็อกว่า "เลขตัวตน = BirthPower ล้วน" ตามตำรา — **ถูกแทนที่แล้ว** เพราะมีผู้ทดลอง
   ยืนยันว่าสูตรรวมตรงกว่า ผู้ใช้จึงสั่งใช้สูตรเดียวทุกโหมด กติกาที่เคาะครบ (31 ส.ค. 2569):
   - สูตร: BirthPower + NamePower + TimePower + DayPower → `reduceTo99` (จุดเดียว:
     `computeCardId` / `personalEnergyNumber` ใน `lib/engine/card-id.ts`)
   - **TimePower ลดทอนเหลือหลักเดียว** (18:30 → 12 → 3) — เปลี่ยนจากเดิมที่ไม่ลด →
     การ์ดผู้ใช้เดิมที่ผลรวมเวลา ≥10 เปลี่ยนใบ (ผู้ใช้รับทราบผ่านการสั่งแก้)
   - **ไม่มีข้อยกเว้นเลขตอง/Master Number** — ลดทอนปกติ (11/22/33 อยู่ใน 00-99 มีการ์ดอยู่แล้ว
     ส่วน 111/222 ไม่มีการ์ดให้เปิด ผู้ใช้เลือกตัดข้อยกเว้นทิ้ง)
   - **โหมดที่ข้อมูลไม่ครบ: ส่วนที่ไม่มี = 0** — องค์รวม/บุคคลที่สาม (มีแค่วันเกิด) ได้ Birth+Day ·
     match เนื้อคู่ส่งเวลา/ชื่อเมื่อมี · `birthPowerNumber` เดิมถูกแทนด้วย `personalEnergyNumber`
   - ตาราง NamePower ไม่เปลี่ยน (Calculation_Constants ที่ verify กับเจ้าของตำราแล้ว รวมอักขระ
     เสริม 7 ตัว + สระประสมนับรายอักขระ — ข้อความสรุปสูตรที่ผู้ใช้ส่งมาตรงกับตารางนี้อยู่แล้ว)
...
3. **digit-bridge (ธาตุจากเลขของการ์ด 00-99) — ฟื้นแบบมีเงื่อนไขเป็น "เลนส์ทางเลือก" (ผู้ใช้เคาะ 1 ก.ย. 2569)**
   เดิมเลิกใช้สนิท — มติใหม่: ธาตุจากเลขตัวตน (artifactElement ของเลขสูตรรวม) กลับมาในฐานะ
   **เลนส์ ข ที่แสดงคู่** กับธาตุกำเนิด (Element Seed = ทาง ก **ยังเป็นแกนหลัก ไม่ถูกแทน**) ·
   ห้ามเฉลี่ยรวมสองเลนส์ (คงข้อ 5) · เลนส์ ข ไม่มีแนวคิดธาตุที่ขาด → ไม่มี Productive Clash
   ในเลนส์นี้ (ประกาศใน IDENTITY_LENS_CAVEAT) · จุดเดียว: `lib/engine/identity-lens.ts`
...
- ~~**Logic 2 — สูตรวิเคราะห์เบอร์โทร**~~ → ✅ **ได้สูตรทางการจากเจ้าของตำราแล้ว 6 ส.ค. 2569**
  (`lib/engine/phone-official.ts` — ตัด 2 หน้า/รายคู่/ถ่วงน้ำหนัก 40-25-15-10) · ตัวเก่า
  `analyzePhoneNumber` คงไว้เพื่อ golden parity เท่านั้น ไม่ใช้ตอบผู้ใช้แล้ว
...
- **Logic 19 — ตาราง 9 กลุ่มอักษร → ธาตุ (`GROUP_TO_ELEMENT`)**: ✅ **เปลี่ยนเป็น "ทาง ค"
  24 ส.ค. 2569 (ผู้ใช้เคาะ)** — เลขกลุ่ม→ดาวประจำเลข (เลขศาสตร์ไทยมาตรฐาน)→ธาตุประจำวัน
  ตามตาราง DAY_ELEMENT ที่ verify แล้ว · ราหู(8)=ดิน เกตุ(9)=ไฟ จากงานวิจัย cross-check
  (คติ Shanivat Rahu/Kujavat Ketu + ทักษาไทยพุธกลางคืน — อนุมานผ่านดาวแม่แบบ) · ผลพวง:
  **ธาตุชื่อไม่มีวันเป็น "ทอง"** (4 ธาตุไทย — precedent เดียวตารางเลข Logic 2) · แก้คู่ .ts/.py
  + regenerate fixtures แล้ว · **ผู้ใช้ยืนยันขั้นสุดท้ายแล้ว 24 ส.ค. 2569** — caveat ทุกจุดเหลือประกาศที่มา (เกณฑ์เลขกลุ่ม→ดาว→ธาตุวันเกิด) ไม่มีคำว่า "รอยืนยัน" อีก · เอกสารทางเลือกเก็บเป็นบันทึก traceability
...
```
อ่าน CLAUDE.md §15 บล็อก "✅ face-card เฟส 1" + "✅ โหมดความรักและเนื้อคู่ v1" + คิวเปิดงาน 10 ส.ค. 2569 แล้วรัน health check (tsc + test + build ควรได้ 483) · รายงานสถานะ · แล้วทำต่อข้อ 3 (เอกสารทางเลือกตาราง 9 กลุ่มอักษร→ธาตุ) หรือทวงงานฝั่งผู้ใช้ข้อ 4
```

### ✅ โหมดความรักและเนื้อคู่ v1 — เสร็จ 21 ส.ค. 2569 (คิวข้อ 1 · ผู้ใช้เคาะ 3 เรื่องในเซสชัน)

**🔴 ข้อค้นพบก่อนสร้าง (ตามที่คิวสั่งให้ตรวจก่อน):** `Zodiac_Signs_csv.csv` = ตาราง ข.2
ตำราจตุพลวัตร **คำต่อคำ** (CSV คือ export ของ ข.2 ไม่ใช่แหล่งแยก — คอลัมน์ zodiac_id คือเลขดาว
เจ้าเรือน ไม่ใช่เลขราศี · กุมภ์ 2 แถวเพราะเจ้าเรือนคู่ ราหู/เสาร์ ตรง ข.2) · ของแถมที่เปิดใช้ครั้งแรก:
`Planet_Meanings_csv.csv` (ความหมายดาว 10 ดวง — ตัดเลขขยะท้ายบรรทัดตอนคัดลอก) ·
**5 ใน 8 หัวข้อโครง Jyotish (รูปลักษณ์/พื้นเพ/ฐานะ/อายุ/ช่วงเวลา) ไม่มีข้อมูลในตำราเลย**

**การตัดสินใจผู้ใช้ (21 ส.ค. 2569 — ห้ามถามซ้ำ):** ① ทำ v1 4 หัวข้อที่มีข้อมูลจริง + ใส่คำถาม
5 หัวข้อที่ขาดในเอกสารถึงเจ้าของตำรา (ต่อท้าย docs/คำถามถึงเจ้าของตำรา_ส.ค.2569.md ชุดที่ 3 แล้ว
— รอส่ง) ② ภาพเนื้อคู่ทำเลยพร้อมป้ายกำกับชัด ③ ราคา: **ฟรีครั้งแรก แล้ว 20 เครดิต** · ชุดภาพ
3 รูป 30 เครดิต (ตามข้อเสนอเดิมในคิว)

**ที่ทำ:**
- `lib/engine/soulmate.ts` (7 เทสต์ → รวม 472): ZODIAC_TRAITS 12 ราศีคัดลอกตรง ข.2 (เทสต์ล็อก
  คำต่อคำ) + PLANET_MEANINGS + `seventhSign()` (ลัคนา→ภพปัตนิ) + `soulmateReading()` (ลัคนา) /
  `soulmateElementReading()` (fallback ไม่มีเวลาเกิด — บอกตรง SOULMATE_NO_TIME_NOTE) + เคมีธาตุ
  ผ่าน wuXingScore มุมเดียวกับ myMatchProfile + ทิศจาก DIRECTION_TO_ELEMENT + `soulmateImagePrompt()`
  (อังกฤษ modest/adult/no-text · โทนสีตามธาตุราศีคู่) · caveat บังคับ 2 ตัว (SOULMATE_CAVEAT +
  SOULMATE_SCOPE_NOTE ระบุครบ 5 หัวข้อที่ยังไม่เปิด — มีเทสต์ล็อก)
- `app/api/soulmate/route.ts`: ล็อกอิน+guest block (แบบ oracle) → ฟรี 1 ครั้ง (bucket logic:17,
  FREE_OVERRIDES) → 20 เครดิต → engine ฿0 → ai2 เรียบเรียง (กฎเหล็กห้ามทำนาย 5 หัวข้อที่ไม่มีข้อมูล)
  → **หักหลังสำเร็จ** · โหมด images: 30 เครดิต ไม่มีสิทธิ์ฟรี → `falSoulmateImages()` (FLUX schnell
  num_images 3, portrait — fn ใหม่ใน fal.ts คืนทุกรูป) + logImageGeneration kind "soulmate_image" ·
  **ไม่มี Safety Gate โดยเจตนา — ฟอร์มไม่รับ free-text** (คำถามต่อไป /api/chat ซึ่งมี gate)
- หน้า `app/(liff)/soulmate/` (marble+triskele): ฟอร์ม prefill จากบัญชี · เพศคู่ที่สนใจบังคับเลือก
  (ห้ามเดา — ไม่เลือก = 400) · แสดงผล+caveat+ปุ่มภาพ (ป้าย "ภาพจินตนาการจาก AI ไม่ใช่บุคคลจริง
  และไม่ได้มาจากตำรา" ใต้ทุกรูป) · FunctionChat logicId 17 · TOOLS หน้าแรก + sitemap
- pricing: `soulmate` ฿0.5→20cr (10.1×) · `soulmate_images` ฿0.66→30cr (11.5×) — ผ่านเทสต์ ≥500% ·
  quota: CHAT_ENABLED_LOGICS + ชื่อ "ความรักและเนื้อคู่" · memory: HistoryKind เพิ่ม "soulmate"
- **verify E2E จริงทั้งเส้น (test user จริง + cleanup):** ลัคนากันย์→คู่มีน ✓ · Productive Clash
  น้ำ +2 ✓ · AI ตอบโครง ①-④ persona ครบ · ครั้งที่ 2 → 429 "ครบ 1 ครั้ง...20 เครดิต" · grant 60 →
  reading −20 → **ยิง FLUX จริงได้ 3 รูป** −30 → ledger ตรงทุกแถว → ลบ user · ภาพตรวจด้วยตา:
  สุภาพ ผู้ใหญ่ โทนเขียวตรงธาตุไม้ที่คำนวณ · browser: ธีม triskele การ์ดครีมถูกต้อง · anon 401 + CTA
- ⚠️ เพศ "ไม่ระบุ" (any) FLUX เลือกเพศภาพเอง (prompt "adult Thai person") — พฤติกรรมยอมรับได้
...
### ✅ ชั้น Jyotish สากลในโหมดเนื้อคู่ — งานวิจัยเต็มรอบ (24 ส.ค. 2569 ผู้ใช้เคาะ "ชุดเต็ม")

**ที่มา:** ผู้ใช้แนบคำอธิบาย Jyotish จาก AI ภายนอก (ภพ 7/เจ้าเรือน/Venus/D9/Upapada/Darakaraka/
Dasha) → ประเมินแล้วเปิดเป็นงานวิจัยแยกรอบ: **ห้ามยกกฎจากแชทบอท** — ค้น+ยืนยันทุกกฎจากแหล่ง
เผยแพร่จริง (BPHS/Jaimini ฉบับแปล · Saptarishis · JagannathHora · Paramarsh ฯลฯ cross-check ≥2
แหล่ง/ข้อ — รายงานเต็ม 2 ชุดในบทสนทนา) + สร้างดาราศาสตร์ใหม่ที่ verify กับ Swiss Ephemeris

**ดาราศาสตร์ใหม่ (`lib/engine/planets.ts` + `scripts/{gen-planet-cases.mjs,verify-planets-swisseph.py}`):**
- พุธ/ศุกร์/อังคาร/พฤหัส/เสาร์ จาก Keplerian elements ของ JPL (Standish 1800-2050) + ราหู mean node
- ✅ **verify 205 เคส ปี 1950-2030: median คลาด 0.004-0.08° · ราศีตรง 1,229/1,230** (พลาด 1 เคส
  ห่างขอบราศี 0.10° = กำกวมขอบ) · ช่อง D9 ตรง ~97-100% · **ดวงจันทร์→นักษัตรตรง 324/324**
- ⚠️ ห้ามใช้แทน jupiterLongitude/saturnLongitude ของ transit.ts (golden test ล็อก) — ชุดนี้สำหรับ Jyotish

**เอนจิน `lib/engine/jyotish.ts` (7 เทสต์ → รวม 511) — ธรรมเนียมที่ล็อกไว้ (มีสำนักอื่น ต้องแจ้งใน caveat):**
- Arudha: นับภพ→เจ้าเรือน แล้วนับเท่ากันต่อ · ข้อยกเว้น ตกภพเดิม→+9, ตกภพ 7→+3 (mainstream software)
- Upapada = arudha ภพ 12 · ภพ 2 จาก UL = ความยั่งยืนชีวิตคู่ (JS 1.4.x — ไม่มีดาว→อ่านเจ้าเรือน+dignity)
- Chara Karaka **แบบ 8 ดวงรวมราหู** (ราหู 30°−องศา) · Darakaraka = องศาน้อยสุด
- D9: floor(lon/3°20′) mod 12 — พิสูจน์เทียบเท่ากฎ จร/คงที่/อุภย ครบ 12 กรณี · dignity (อุจ/เรือนตัวเอง)
- Vimshottari: เกตุ7 ศุกร์20 อาทิตย์6 จันทร์10 อังคาร7 ราหู18 พฤหัส16 เสาร์19 พุธ17 · เศษทศาแรกตาม
  สัดส่วนนักษัตรที่เหลือ · อันตรเริ่มที่เจ้ามหาทศา · ปี 365.25 วัน · เรือน whole-sign · กุมภ์=เสาร์ (คลาสสิก
  ไม่ใช่ราหูแบบไทย — เทสต์ล็อก) · จังหวะเรื่องคู่ = MD/AD ของ {เจ้าเรือน 7, ศุกร์, ดาวในภพ 7, เจ้าเรือน UL,
  DK} (+พฤหัสถ้าผู้ใช้หญิง) แสดง 3 ช่วงใน 8 ปี — MD+AD เข้าชุดทั้งคู่มาก่อน
- ตารางไทย: SEVENTH_LORD_ARENA_TH 12 ภพ (บริบทพบคู่) · PLANET_IN_7TH_TH 9 ดาว · DK_ARCHETYPE_TH ·
  ⚠️ ตารางลักษณะ = ธรรมเนียมนักปฏิบัติต่อยอดคัมภีร์ ไม่ใช่โศลกคำต่อคำ — JYOTISH_CAVEAT +
  JYOTISH_TIMING_CAVEAT บังคับ (เทสต์ล็อก)
- **wiring:** /api/soulmate โหมด reading คำนวณเมื่อมีเวลาเกิด (฿0 · พังไม่ล้มคำทำนายหลัก — try/catch) ·
  context "ชั้นJyotishสากล_ชั้นเสริม" + กฎ narrator ③.3 "มุมมองจากดวงดาว" (ห้ามขัด/แทนชั้นตำรา ·
  จังหวะเวลา = น้ำหนัก ไม่การันตี) · **กฎข้อ 2 เปลี่ยน: "บริบทที่พบคู่ + จังหวะเวลา" เปิดผ่านชั้นนี้แล้ว**
  (SOULMATE_SCOPE_NOTE เหลือปิด 3: พื้นเพ/ฐานะ/อายุ) · หน้า /soulmate ส่วน <details> 🪐 + ส่ง
  userGender จากโปรไฟล์ (useStoredProfile เพิ่ม gender)
- **verify E2E จริง (test user + AI + cleanup):** ดวง 7 ต.ค. 2529 06:28 ลพบุรี → ภพ 7 มีน · พฤหัสภพ 6 ·
...
- **ผสานสองศาสตร์เป็นพอร์เทรตเดียว (ผู้ใช้เคาะ 24 ส.ค. 2569 — ทดลอง 5 ดวงก่อนแล้วจูน 2 จุด):**
  ① กติกาลำดับชั้นทางการใน narrator: ตำรา=ภาพหลักเสมอ · Jyotish=ตัวเติม · ขัดกัน→เล่ามุมสากลเป็น
  "อีกแง่มุม" ห้ามเฉลี่ย/เลือกข้างสากล ② โครงคำตอบใหม่ = พอร์เทรตเดียวต่อเนื่อง (นิสัยถักสองชั้น →
  รูปลักษณ์ ค.1+กลิ่นอายดาว → เคมี+ความยั่งยืน → บริบท+จังหวะ → ชื่อ → caveat) + กฎห้ามแต่งสเกล
  ("2/5" — จับได้จากทดสอบจริง) ③ `soulmateConvergence()` (jyotish.ts) ตัวชี้ความสอดคล้อง 3 สัญญาณ
  (เคมีตำรา ± · Upapada tone · D9 strength) นับเสียงส่วนใหญ่แบบ Logic 3 — **ไม่เฉลี่ยคะแนนข้ามระบบ**
  (คงกติกา §4 ข้อ 5) · ป้าย 4 แบบ: สอดคล้องบวก/สอดคล้องต้องดูแล/ต่างมุม/**ตำราเป็นเสียงหลัก**
  (จูนข้อ 1 — เดิม "โทนกลาง" กลบเคมีตำราที่ชี้อยู่จริง) · จูนข้อ 2: เพิ่ม**ราศีนิจ** (ตรงข้ามอุจ —
  ตารางคลาสสิก) ให้ D9 ชี้ลบได้จริง (เดิมระบบเอียงบวก โครงสร้างไม่มีทางออกป้ายลบ) · Dignity เพิ่ม
  "debilitated" + UlTone/D9Strength เป็น structured code · แถว "🧭 ความสอดคล้องระหว่างศาสตร์"
  บนหน้า + **ระบบแนบบรรทัดนี้ต่อท้ายคำตอบ AI เองแบบ deterministic** (Flash ข้ามบรรทัดบังคับได้ —
  พิสูจน์ 2 รอบ · แพทเทิร์นเดียว caveat) · เทสต์ 514 · E2E จริง: ดวงกันย์ 2529 → ต่างมุม
  (เคมี+ UL− D9+) บรรทัดแนบ 1 ครั้งเป๊ะ ไม่มีสเกลแต่งเอง
- **✅ งานค้างปิดครบ (24 ส.ค. 2569 ผู้ใช้สั่ง "ดำเนินการให้แล้วเสร็จ"):**
  ① **Bhavat Bhavam (derived houses)**: `signTone()` (กลไกเดียว UL2) + `derived` ใน SoulmateJyotish —
  ทรัพย์/ครอบครัวฝั่งคู่=ภพ 8 (2จาก7) · การงานคู่=ภพ 4 (10จาก7) · รากฐานพื้นเพคู่=ภพ 10 (4จาก7)
  โทนจากดาวสถิต+dignity เจ้าเรือน · **SOULMATE_SCOPE_NOTE เหลือปิดหัวข้อเดียว: "อายุ"**
  (พื้นเพ/ฐานะ/การงานเปิดเป็นแนวโน้มกว้างๆ ชั้นเสริม — narrator ห้ามระบุตัวเลขทรัพย์/อาชีพเจาะจง)
  ② **โหมด match ได้ชั้น Jyotish**: อีกฝ่ายมีเวลาเกิด → คำนวณดวงเขาชั่วขณะ (**ไม่จัดเก็บ** —
  E2E ยืนยัน history ไม่มีวันเกิดเขา) → `overlapWindows()` จุดตัด+รวมช่วงต่อเนื่อง → บล็อก
  "🪐 จังหวะเวลาสองฝ่าย" (ช่วงของคุณ/ของเขา/ทับซ้อน) + context + กฎ narrator 4.5 (ห้ามการันตี) ·
  windows เพิ่ม fromMs/toMs · E2E จริง: กันย์2529×ธนู2535 → ทับซ้อน พ.ค.2574–ส.ค.2577 ถูกต้อง
  ③.5 **ตาราง 9 กลุ่มอักษร → "ทาง ค" มีผลจริงแล้ว (ผู้ใช้เคาะ + สั่งวิจัยราหู/เกตุ):** ดูธง §5
  ฉบับแก้ — สมชาย ทอง→**ดิน** · รุ่งเรือง ไม้→**น้ำ** · น้ำทิพย์ ไม้→**ไฟ** (เทสต์ chat-plan/
  logo-team/timing ปรับตาม · 516 ผ่าน) · เอกสารทางเลือกเปลี่ยนเป็น "ใช้แล้ว รออาจารย์ยืนยัน/ทักท้วง"
  ③ **เอกสารทางเลือกตาราง 9 กลุ่มอักษร (คิวข้อ 3 เดิม)**: `docs/ทางเลือกตาราง9กลุ่มอักษร_ส.ค.2569.md`
  — 3 ทาง (ก ปัจจุบัน/ข ฐานเสียงวรรค/ค เลข→ดาว→ธาตุวันเกิด) + ตัวอย่างผลจริงจาก engine (สมชาย=ทอง
  40% ฯลฯ — verify แล้ว ไม่เดา) + ช่องติ๊ก · **รอผู้ใช้ส่งเจ้าของตำรา** · เทสต์ 516

### ✅ โหมด "ทำนายแบบองค์รวม" (Logic 20 ยกเครื่อง) + ตัด /lucky-number — 22 ส.ค. 2569 (ผู้ใช้สั่งจาก screenshot)

**คำสั่งผู้ใช้:** ① ตัดโหมดดูทะเบียนรถ/เลขที่บ้าน (/lucky-number) ออก ② ยกเครื่อง /compatibility
ให้เพิ่มหลายรายการ (ตนเอง/บ้าน/ทะเบียนรถ/โทรศัพท์/เพื่อนร่วมงาน) → **คะแนน 5 ด้านของแต่ละส่วนก่อน**
→ ความสอดคล้องทั้งหมด → จุดแข็ง/ข้อควรระวัง(ถ้ามี)/ข้อแนะนำอื่น ③ เปลี่ยนชื่อโหมดเป็น **"ทำนายแบบองค์รวม"**

**ที่ทำ:**
- `lib/engine/network-holistic.ts` (6 เทสต์ → รวม 483, ฿0 ล้วนทั้งโหมด):
  · `birthPowerNumber()` — "เลขตัวตน" ของส่วนตนเอง = **BirthPower ล้วน→reduceTo99** (นิยาม
    เลขตัวตน 00-99 ตาม §4 ข้อ 1 — ไม่ใช่สูตรการ์ด A เต็มเพราะไม่พึ่งชื่อ/เวลา)
  · `parseRefInput()` — รับ "จง 6266"/เบอร์ 0 นำหน้า/บ้านเลขที่ (แยกอักษรป้ายเข้า plateLetters)
  · `partAspects()` ห่อ numberAspects จุดเดียว (เทสต์ล็อก deepEqual — หน้าห้ามคำนวณเอง)
  · `analyzeCoherence()` — ต่อด้าน: avg/min/max/ส่วนเด่นสุด-อ่อนสุด · tone: strong (ทุกส่วน ≥6.5) /
    caution (มีส่วน ≤4) / neutral
  · `holisticAdvice()` — เทมเพลตจากข้อมูลจริง: จุดแข็ง (ด้าน strong + เคมี +2 + Productive Clash
    "พลิกเป็นยา") · ข้อควรระวัง (ด้าน caution + ส่วนที่พยุงได้ · เคมีพิฆาต → **ธาตุสะพาน 通關**
    `bridgeElement()` = ธาตุที่ผู้พิฆาตให้กำเนิด (ไฟ↔ทอง→ดิน, เทสต์ล็อก) + สีจาก ELEMENT_TO_COLORS)
    · caveat บังคับ 2 ตัว (NUMBER_ASPECTS_CAVEAT + HOLISTIC_CAVEAT — เทสต์ล็อก)
- **/compatibility ยกเครื่อง**: ชื่อใหม่ทุกจุด (h1/TOOLS/quota/pricing/metadata — **router.ts ไม่แตะ**
  ล็อก golden) · Entity เพิ่ม type "phone" 📱 + เก็บ `ref` ดิบ · ช่องเลขอ้างอิงเป็น text (คงเลข 0
  นำหน้า+อักษรป้าย) · ส่วนตนเองโชว์เลขตัวตน+แถบ 5 ด้าน · คลิกรายส่วน → แถบ 5 ด้าน+การ์ดผลรวม+เลขท้าย
  · ส่วนที่ 4 ใหม่: ตาราง coherence 5 ด้าน (ชิป ✅/⚠️/กลางๆ + เฉลี่ย/สูงสุด/ต่ำสุด) + จุดแข็ง/ระวัง/แนะนำ
  · CSS สีตัวหนังสือ**ตายตัวเข้ม** ทุกแถบคะแนน (บทเรียน /lucky-number จาง — screenshot ผู้ใช้)
- **/lucky-number ตัดออก**: ลบหน้า + **301 → /compatibility** (next.config redirects — รักษาอันดับ/
  ลิงก์เดิมที่เคยอยู่ sitemap+GSC) · metadata /compatibility รับช่วงคีย์เวิร์ด "ทะเบียนรถ เบอร์โทร
  บ้านเลขที่" (เป้า 15.9k/เดือน KD 2 เดิม) · แก้ลิงก์ใน /ai · sitemap ถอด+อัป /compatibility 0.9
- **verify browser จริงทั้ง flow**: เกิด 1986-10-07 → เลขตัวตน 32 (ตรงมือ: 7+1+24) · เพิ่ม 3 ส่วน
  (จง 6266 · บ้าน 47 · เบอร์ 0812345678) · **ตัวเลขบนจอตรง engine เป๊ะทุกค่า** (จง6266:
  9.5/10/6.1/7.6/5.4 ภาพรวม 7.7) · coherence 5 แถว + ระวัง "บ้านด้านสุขภาพ 3.5 ฉุดข่าย" + แนะนำ
  "รถแข็งแรงสุด 7.7 ใช้เป็นหลักยึด" + caveat ครบ · redirect 308 จริง · เทสต์ 483 · build ผ่าน
- ⚠️ Browser pane ค้างช่วงท้าย (screenshot เฟรมเก่าดำ) — ส่วน 3-4 ยืนยันผ่าน getComputedStyle/DOM
  ตามบทเรียนเครื่องมือเดิม (ส่วน 1 เห็นภาพจริงแล้วว่าอ่านชัด — คอมโพเนนต์แถบเดียวกันทุกจุด)
- **ฟอร์มหลายแถว + ปุ่ม "รับคำทำนายในภาพรวม" (ผู้ใช้เคาะ 22 ส.ค. 2569):** เพิ่มสิ่งรอบตัวได้
  **สูงสุด 10** (ปุ่ม ➕ เพิ่มข้อมูลสิ่งรอบตัว — แถวการ์ดมีเลขที่+ปุ่ม ✕ · ชื่อเรียกไม่บังคับ
  auto-name จากประเภท+เลข) · **ฟรี 2 รายการแรก (ไม่ต้องล็อกอิน — คงแม่เหล็ก §12) · เกิน 2 =
  20 เครดิต** ผ่าน `/api/holistic` (key `holistic_network` costThb 0 — ขายมูลค่า · บัญชีถาวรเท่านั้น
  · **หักครั้งเดียวต่อเซสชันเบราว์เซอร์** sessionStorage `kruth_holistic_unlock` — ปรับรายการแล้ว
  ทำนายซ้ำไม่หักซ้ำ) · ปุ่ม "เพิ่มเข้าข่าย" → **"รับคำทำนายในภาพรวม"** ยังไม่กดคำนวณธาตุก็กดได้
  (auto-คำนวณตัวคุณก่อนแล้วทำนายต่อ) · prefill วันเกิดจากบัญชี (useStoredProfile) ·
  ค่าคงที่ FREE_NETWORK_PARTS=2 / MAX_NETWORK_PARTS=10 ใน network-holistic.ts (เทสต์ล็อก 484)
  ⚠️ ข้อจำกัดที่จดตรงๆ: engine โหมดนี้อยู่ใน client bundle (เส้นฟรีใช้ร่วม) — เกตคุม UI ปกติ
  ไม่ใช่กันแกะ bundle · **verify จริง:** browser ฟรี 2 รายการ+auto-self+auto-name ครบ · anon
  เกิน 2 → 401 CTA · API E2E: ขอบเขต 3-10 · 429 · grant 30→หัก 20 ledger ตรง → ลบ user ·
  🐛 บั๊กที่จับได้ตอนทดสอบ: กดเพิ่มแถวรัวใน tick เดียวทะลุเพดาน (guard อ่าน state เก่า) →
  ย้ายเช็ค+ออก id เข้า functional update
- **บุคคลด้วยวันเกิด + จังหวะเริ่มต้น + คำทำนาย 4 องก์ (ผู้ใช้เคาะ 22 ส.ค. 2569 — "ดำเนินการเลย"):**
  ① ประเภทบุคคล (เพื่อนร่วมงาน/คู่รัก — `isPersonType`) ฟอร์มสลับเป็น**ช่องวันเกิด**แทนเลข:
  ธาตุจาก `personSeedFromBirthDate()` (ElementSeed สูตรคนตัวจริง) + คะแนน 5 ด้านจาก**เลขตัวตน
  ของเขา** (birthPowerNumber — วิธีเดียวกับส่วนตนเอง ไม่มีสูตรใหม่) + แถว "ธาตุที่เขาขาด" +
  โน้ตบังคับ "อ่านเป็นความเข้ากันของพลังงาน ไม่ใช่คำตัดสินตัวบุคคล" · วันเกิดบุคคลที่สาม**ไม่ขึ้น
  server เลย** (เส้นฟรี client ล้วน · ปลดล็อกส่งแค่ count)
  ② วัตถุเพิ่มช่อง **วันเริ่มต้น+เวลา (ไม่บังคับ)** → `startDateOmen()` กาลโยคย้อนหลัง
  (checkDayKalaYoke + ขอบเขต จ.ศ. 16 เม.ย. ผ่าน `kalaYokeCsForDate` ที่ export จาก timing.ts ·
  มีเวลา = เพิ่มยามผ่าน checkCombinedAuspiciousTime) — แสดงเป็นแถว "🏁 จังหวะเริ่มต้น" + เข้า
  จุดแข็ง/ระวังใน holisticAdvice · 🔴 **ห้ามเอาวันที่ไปสร้างธาตุวัตถุ** (ตำราไม่มีกฎ + จะชน 2 ธาตุ
  ต่อชิ้น) · ผลร้ายเฟรมนุ่มตามหลักตำรา "ไม่ได้ร้ายทั้งวัน" (เทสต์ล็อก) + START_OMEN_CAVEAT
  (=TIMING_CAVEAT) ติดมาเมื่อมี omen เท่านั้น
  ③ **คำทำนาย 4 องก์จากอาจารย์ลาลา — เฉพาะเส้นปลดล็อก (>2 ชิ้น)**: /api/holistic เพิ่ม mode
  "narrate" (ai2 ~฿0.3-0.5/ครั้ง ไม่หักเครดิตเพิ่ม — รวมใน 20cr แล้ว · ลง ai_usage_log อัตโนมัติ)
  โครง: ตัวคุณ→รายส่วน→ความสอดคล้อง→ทางปฏิบัติ · กฎเหล็ก: เลขจาก context เท่านั้น/x/10 เสมอ/
  ชู Productive Clash/บุคคลห้ามตัดสินนิสัย/ผลร้ายจบด้วยทางแก้ · client เรียกผ่าน useEffect
  guard ด้วย key (ยิงครั้งเดียวต่อชุดผล) แสดงเป็นส่วนที่ 5 · เส้นฟรีเห็น upsell แทน ·
  **verify E2E จริง:** anon 401 · ข่ายฟรี 400 · AI จริง 6.1 วิ ได้ 4 องก์ตัวเลขตรง context ·
  browser: ฟอร์มสลับช่องตามประเภท + omen row + ส่วนบุคคลครบ (เทสต์ 487)
- **กาง/พับผลลัพธ์ (ผู้ใช้ขอเพิ่ม 22 ส.ค. 2569):** ทุกบล็อกผลพับได้ — แถบ 5 ด้านของตนเอง /
  🕸 แผนผังกราฟ+คะแนนรวม / ตารางความสอดคล้อง / จุดแข็ง-ระวัง-แนะนำ ใช้ `<details>` พื้นเมือง
  (คลาส `.fold` — default เปิด) · **รายส่วนเป็นแอคคอร์เดียน** (state Set ของ id — หัวแถวโชว์สรุป
  ไอคอน+ชื่อ+ภาพรวม x/10+เคมีธาตุ · กางแล้วเห็นแถบ 5 ด้าน+การ์ดผลรวม+เลขท้าย · คลิกโหนดใน SVG
  = force-open ส่วนนั้น) — แทนกล่อง detail เดี่ยวแบบเดิม · verify browser จริง: กาง/พับ/คลิกโหนด/
  ลบรายการ ครบ (⚠️ เกร็ด: `<details>` ปิดใน Chrome — getBoundingClientRect ลูกยังคืนขนาดเดิม
  ต้องเช็คด้วย `checkVisibility()`)
- ⚠️ ผลพวง SEO ที่ต้องรู้: หน้า SEO เฟส 1 "ดูดวงทะเบียนรถ" หายไปตามคำสั่ง — เนื้อหา static
  ยาวของหน้านั้นไม่ได้ย้ายมา (หน้ /compatibility เป็นเครื่องมือ ไม่ใช่บทความ) ถ้าอันดับตกและ
  อยากได้คืน ให้พิจารณาเพิ่มบล็อกเนื้อหา static ใต้เครื่องมือใน /compatibility
...
### ✅ /timing ช่องกรอกรายหมวด + ปรับเมนูหน้าแรก — 22 ส.ค. 2569 (ผู้ใช้สั่งชุดท้าย)

- **เมนูหน้าแรก (ผู้ใช้เคาะจาก screenshot):** ถอด "ดวงของฉัน" (/fortune ยังอยู่ — SEO/ลิงก์ตรง) ·
  เพิ่ม "📅 ดูฤกษ์ดี" (/timing ไม่เคยมีปุ่ม) + "🎨 สร้างโลโก้ตามดวง" (/logo หายจากเมนู) ·
  ปุ่มเนื้อคู่/องค์รวมมีอยู่แล้วหลัง deploy (screenshot ผู้ใช้เป็นแคชก่อน deploy)
- **/timing ช่องเสริมรายหมวด (ทุกช่องไม่บังคับ — ไม่กรอก = ผลเท่าเดิมเป๊ะ มีเทสต์ deepEqual ล็อก):**
  `ACTIVITY_FIELDS` ใน timing.ts — เปิดบริษัท→ชื่อกิจการ (ธาตุชื่อ Logic 19 ⚠️ตารางยังไม่ verify →
  caveat ติดอัตโนมัติ) · ขอทะเบียนรถ→เลขทะเบียน · ขึ้นบ้านใหม่→บ้านเลขที่ (ธาตุวัตถุ Logic 2) ·
  เจรจา→วันเกิดคู่เจรจา · ทุกหมวด→วันเกิดผู้ทำ (prefill จากบัญชี)
- **ชั้นคำนวณที่เพิ่ม (ของจริงทั้งหมด):** วันเกิด → (1) **กาลกิณี**: จันทร์จร ณ ~เที่ยงไทย (hour 5 UTC)
  เข้าเรือนดาวกาลกิณี (`kalakiniRuledSigns` TS-wrapper ใหม่ใน daily.ts + getMoonSign) → −3 +
  นับเป็นสัญญาณร้ายใน verdict (หลักเดียวกับ Logic 8) · เกิดวันศุกร์ (ราหู) ตรวจไม่ได้ → caveat บอกตรง
  (2) ธาตุประจำวัน (DAY_ELEMENT verify แล้ว) × ธาตุผู้ทำ/คู่เจรจา ±1 · วัตถุ/ชื่อ: มุม "วันบำรุงของ"
  wuXingScore(ของ, วัน) ±1 — น้ำหนักเบากว่ากาลโยค (±3) โดยเจตนา · `DayRanking.personalNotes`
  โชว์เหตุผลรายวัน · caveat ประกาศชั้นที่รวม/ยังไม่รวม (ลัคนารายชั่วโมงยังไม่รวม)
- **verify:** เทสต์ 491 (timing +4: no-input identical · กาลกิณี wiring เทียบ engine ตรงทุกวันใน 45 วัน ·
  ราหู · fields ครบทุกหมวด+caveat ชื่อ) · browser จริง: ช่องสลับตามหมวด · โน้ต "ลมเกื้อหนุนไฟ +2" ·
  กาลกิณีโผล่ในวันควรเลี่ยง · เมนูครบ 10 ปุ่ม
- 🐛 เกร็ดเครื่องมือ: navigate ของ Browser pane บางจังหวะพา tab กลับ "/" — ใช้ location.assign
  + เช็ค location.pathname ก่อนอ่าน DOM ทุกครั้ง

### ✅ /logo ทีมเจ้าของ+หุ้นส่วน + ทิศสำนักงาน + กล่อง Prompt — 23 ส.ค. 2569 (ผู้ใช้เคาะ)

- `lib/engine/logo-team.ts` (6 เทสต์ → รวม 497): `teamMemberFromBirthDate` (ElementSeed สูตรคนจริง ·
  พ.ศ.=null) · `scoreStylesForTeam` — คะแนนสไตล์ 5 ธาตุรายคน + **teamMin (ต่ำสุดของทีม — สไตล์ที่ดี
  ต้องไม่พิฆาตใครสักคน โดยเจตนา ไม่ใช่เฉลี่ย)** + ธาตุชื่อแบรนด์ (nameElement ⚠️caveat ตารางยังไม่
  verify ติดอัตโนมัติ) + ทิศ (DIRECTION_TO_ELEMENT — "ทิศบำรุงสไตล์" wuXingScore(สไตล์,ธาตุทิศ)) →
  `recommended` (เรียง teamMin→ทิศ→ชื่อ · ไม่มีข้อมูล=null ไม่เดา) · `directionOwnerAdvice` —
  ทิศพิฆาตธาตุเจ้าของ → แนะสีธาตุสะพาน 通關 ในโลโก้ (bridgeElement+ELEMENT_TO_COLORS) ·
  clash=ชูเป็นยา · `OFFICE_DIRECTIONS` 8 ทิศ (ไม่มี "กลาง") · `OFFICE_DIRECTION_HELP` วิธีดูทิศ
  4 ข้อ (ยืนที่ประตูมองออก/แอปเข็มทิศ/Google Maps เหนือบนเสมอ/มุมตึกยึดประตูลูกค้า — ผู้ใช้ขอ)
- **หน้า /logo**: ช่องวันเกิดเจ้าของ (prefill จากบัญชี) + ➕หุ้นส่วนสูงสุด 4 (MAX_TEAM_MEMBERS=5) +
  dropdown ทิศ + <details> วิธีดูทิศ + 🧭 คำแนะนำทิศ↔เจ้าของ · ชิปสไตล์ใช้ teamMin + ⭐สไตล์แนะนำ +
  บรรทัด breakdown รายชั้น (เจ้าของ/หุ้นส่วน/ชื่อแบรนด์/ทิศ) · default สไตล์ = recommended ·
  **กล่อง Prompt สด** (logoImagePrompt ตัวเดียวกับ /api/logo) + ปุ่มคัดลอก + คำอธิบาย "วางใน AI
  เจ้าอื่น (ChatGPT/Gemini/Midjourney/Canva) สร้างเองได้" — ผู้ใช้ขอ · แก้ warning border
  shorthand+borderColor ปน (chip active)
- **ไม่เพิ่มชื่อ-นามสกุลเจ้าของเป็นตัวคำนวณ** (ผู้ใช้รับข้อเสนอ): ไม่มีกฎตำราเชื่อมชื่อบุคคล→โลโก้ —
  เปิดใช้ธาตุ "ชื่อแบรนด์" ที่มีเอนจินจริงแทน · ทุกชั้นคำนวณฟรี ฿0 ราคาเจนภาพเท่าเดิม
- **verify browser จริง**: รุ่งเรืองกาแฟ→ธาตุไฟ · เจ้าของไฟ+หุ้นส่วน+ตะวันตก → 🧭 "ทองพิฆาตไฟ
  แนะสีดิน (เหลือง/น้ำตาล/ครีม) 通關" · ⭐แนะนำเปลี่ยนตามทีมจริง · breakdown "เจ้าของ +1 · หุ้นส่วน 1
  +1 · ชื่อแบรนด์ +1 · ทิศ +1" · prompt ตรง logoImagePrompt + ปุ่มคัดลอก

### ✅ /logo prompt ภายนอกแยกเวอร์ชัน — 23 ส.ค. 2569 (feedback ผู้ใช้หลังลองจริง)

- **ผู้ใช้ลองคัดลอก prompt ไปใช้ใน ChatGPT/Gemini แล้วเจอ:** เรียบเกิน · สีเดียว · ไม่มีตัวอักษร —
  เพราะตัวเดิม (logoImagePrompt) จูนสำหรับ pipeline เรา: minimal+สีเดียวคุม FLUX ง่าย + **no-text
  โดยเจตนา** (วางชื่อทับด้วยฟอนต์ไทยจริง)
- แก้ด้วย `logoExternalPrompt()` ใน naming.ts (TS-only · เทสต์ล็อก 498): ดีไซน์มีมิติ (rich layered/
  not flat) + พาเลตหลายสีตามธาตุ (LOGO_PALETTE_EN สีหลัก+รอง+เน้น) + **สั่งใส่ชื่อแบรนด์เป็น
  typography ในภาพ** ("spelled exactly as written") · 🔴 **ห้ามเอาเวอร์ชันนี้ยิง fal** — ตัวภายใน
  ต้อง no-text เหมือนเดิม (เทสต์ล็อกทั้งคู่)
- กล่อง Prompt บนหน้า /logo ใช้เวอร์ชันภายนอกเป็นหลัก + ทิป "ชื่อไทยอาจสะกดเพี้ยน — สั่ง AI แก้ได้
  หรือขอแบบไม่มีตัวอักษรแล้วเติมเอง" + <details> ดูเวอร์ชันภายใน (ไอคอนล้วน) ประกอบ

### ✅ ภาพเนื้อคู่ v3: คอลลาจรูปเดียวหลายอิริยาบถ + แต่งกาย/อารมณ์ตามคำทำนาย — 23 ส.ค. 2569

- **ผู้ใช้แนบภาพ meme คอลลาจ (คนเดียวหลายท่า) เป็นแนวทาง + สั่งให้ชุด/อารมณ์ภาพตามคำทำนาย** —
  ปัญหาเดิมของ 3 ภาพแยก: FLUX ให้คนละหน้า 3 คน (ขัดคอนเซ็ปต์ "เนื้อคู่คนเดียว")
- `soulmateCollagePrompt()` (soulmate.ts): คอลลาจ 2x2 คนเดียว 4 ท่า/มุม (close-up/side profile/
  นั่ง/ยืนกอดอก) + "exactly one person per panel" (รอบแรก FLUX ยัด 2 คนในช่องเดียว — แก้แล้ว
  ทดสอบจริงผ่าน) + identical face/hair/outfit + นรลักษณ์ ค.1 + ลุคสัญชาติ + **OUTFIT_MOOD_BY_ELEMENT**
  (ชุด+อารมณ์ภาพ 5 ธาตุ แปลจากบุคลิก ข.2/ค.1 เป็นภาษาภาพ — modest บังคับ) + ผิว/รูขุมขน + no-text
- API images mode: เจน **1 รูป** (เดิม 3) — ต้นทุน fal ลด ~3× ราคาเดิม 30cr · response เพิ่ม
  `captions[]` แยกจากรูป (คำบรรยาย 3 บรรทัดจาก engine แสดงเป็นลิสต์ใต้คอลลาจ) · แถว DB
  image_paths 1 ตัว — หน้า /sm รองรับทั้งแถวใหม่ (1 รูป = captions ครบชุดใต้รูป) และแถวเก่า 3 รูป
...
- **ชั้นเสริมธาตุจากชื่อผู้ใช้ (ผู้ใช้เคาะ 23 ส.ค. 2569):** ช่อง "ชื่อ-นามสกุลของคุณ" (ไม่บังคับ)
  prefill จาก first_name+last_name ของบัญชี (แพทเทิร์นเดียววันเกิด) → `buildOwnNameLayer`
  (soulmate.ts): nameElement (Logic 19) ↔ ธาตุคู่ ด้วย wuXingScore มุมเดียวเคมีหลัก + Productive
  Clash ได้ · **เป็นชั้นเสริมเท่านั้น** (แกนหลักยังลัคนา/ภพปัตนิ — ตารางอักษรรอเจ้าของสูตรยืนยัน
  → NAME_TABLE_CAVEAT ติดเสมอเมื่อมีชั้นนี้ เทสต์ล็อก) · ทั้งโหมดลัคนา+โหมดธาตุ · แถวผล
  "ธาตุจากชื่อคุณ↔ธาตุคู่ ⚠️" + เข้า context narrator · ไม่มีชื่อ = ผลเท่าเดิมเป๊ะ (เทสต์ล็อก) ·
  สิทธิ์ฟรีไม่เปลี่ยน (ดู 1 + ภาพ 1 ยังฟรี — E2E ยืนยันทั้งคู่ paid?=false + nameLayer ไม้↔ไฟ +1)
- **ชั้นชื่อขยายเป็นคำทำนายเต็ม (ผู้ใช้เคาะ 23 ส.ค. 2569 รอบสอง):** OwnNameLayer เพิ่ม
  `namePower` (สูตร verify แล้ว) + `card` (reduceTo99 → ตาราง 00-99 จริง — กลไกเดียวการ์ดผลรวม
  สายเลข) + `lens` "เนื้อคู่มุมธาตุชื่อ": ธาตุคู่อันดับ 1 จาก wuXingScore(ธาตุชื่อ, ทุกธาตุ, ที่ขาด)
  → นิสัย (ELEMENT_PERSONA) + รูปลักษณ์ (ค.1 จริง) + สไตล์ (OUTFIT_MOOD_TH ใหม่ — ไทยคู่กับตัว En
  ของภาพ) · โหมด match ได้ namePower+card ของชื่อเขาด้วย แต่**จงใจไม่มี lens** (ไม่ทำนายหน้าตา/นิสัย
  คนจริงจากชื่อ — การตัดสินใจเดิม) · narrator ทั้งสองโหมดเพิ่มหัวข้อ "พลังจากชื่อ" (ชั้นเสริม ห้ามขัด/
  แทนแกนลัคนา — E2E: AI เล่าครบ การ์ด 38+มุมธาตุน้ำ+ค.1+สไตล์ ตรง engine เป๊ะ) · แถวผลบนหน้า
  ครบทั้ง reading+match · เทสต์ล็อกทุกค่ามาจาก engine จริง (505)
...
- **เก็บตกตามผู้ใช้ทัก (23 ส.ค. 2569):** ① promptEn นรลักษณ์ครบวลีรูปร่างทั้ง 5 ธาตุ (Water เพิ่ม
  ...
  ตัวอักษร รูปลักษณ์หายทั้งท่อน — split ที่ " · แนวโน้มรูปลักษณ์ตามนรลักษณ์: ") ③ ภาพชิดบน:
  objectPosition top ใน OG/สตอรี่ + prompt "fills most of each panel, head near the top edge"
  (แก้ต้นเหตุช่องเต็มตัวคนเล็กชิดล่าง — ยิง FLUX ยืนยันเต็มช่องจริง) ④ copy หน้า /soulmate
  เปลี่ยน "3 รูป" → "คอลลาจ 1 รูป 4 มุม" · ตัวเลือกสัญชาติ 8 แบบคงเดิม

### ✅ ภาพเนื้อคู่ v2: สว่างสมจริง 3 ฉาก + คำบรรยายรายภาพ + แชร์ /sm/<token> — 23 ส.ค. 2569

- **feedback ผู้ใช้หลังลองจริง:** ภาพมืด/จัดฉาก/สีเดียว — ต้นเหตุคือ ELEMENT_MOOD เดิมเอาสีธาตุ
  คุมโทนทั้งภาพ (ธาตุน้ำ = teal ทั้งรูป) + "cinematic/dreamy/artistic illustration" ใน prompt
- **แก้ (soulmate.ts):** prompt ใหม่ bright natural photorealistic + สีธาตุเหลือแค่ **สีเน้น**
  ในเสื้อผ้า/ฉาก (ELEMENT_ACCENT) · **3 variant ฉาก** (หน้าต่างเช้า/ไลฟ์สไตล์/สวนกลางแจ้ง) —
  fal ยิงขนาน 1 prompt/รูป ต้นทุนเท่าเดิม · `soulmateImageCaptions()` คำบรรยายประจำภาพจาก
  engine ล้วน: ภาพ1=นิสัย ข.2 · ภาพ2=จุดแข็ง ข.2 · ภาพ3=เคมี wuXing+ทิศ (เทสต์ล็อกคำต่อคำ)
- **แชร์โซเชียล (ผู้ใช้ขอ):** migration 042 `soulmate_gen_e` (รัน prod แล้ว + NOTIFY pgrst ตาม
  บทเรียน PGRST205/205) + bucket private `soulmate_images` + `lib/soulmate/store.ts` (token ใช้ร่วม
  face-card) · เก็บชุด 3 ภาพ+captions ต่อแถวเดียว → หน้าแชร์ `/sm/<token>` (แสดงจริงทุกคน ครบ
  3 ภาพ+คำบรรยาย+ป้าย AI ทุกรูป+CTA) + OG ซ้ายข้อความ(หัวเรื่อง/นิสัยคู่/CTA/ป้าย AI บังคับ)
  ขวาภาพแรกเต็มครึ่ง · ปุ่มคัดลอกลิงก์+native share บนหน้า /soulmate · robots /sm/ แบบเดียว /s/ ·
  next.config trace ฟอนต์ · ลบบัญชี → deleteSoulmateImages ด้วย
- **verify E2E จริง (FLUX 3 ภาพ 11.9 วิ −30cr):** captions ตรง ข.2/Productive Clash · /sm 200 ·
  OG 200 ตรวจด้วยตา (เลย์เอาต์+ป้ายครบ) · token ปลอม 404 · ลบบัญชี → ไฟล์ 0 + แชร์ 404 ·
  ภาพจริงสว่างธรรมชาติ สีธาตุเป็นสีเสื้อ (เทสต์ 500)
- ~~ประเมินโหมด "เช็คกับคนที่สนใจ"~~ → ✅ **ผู้ใช้เคาะ "เริ่มดำเนินการ" — เสร็จ 23 ส.ค. 2569:**
  `partnerMatchReading()` ใน soulmate.ts (เทสต์ +2 → รวม 502) ประกอบจากชิ้นเดิมทั้งหมด:
  ธาตุ+เลขตัวตนเขา (personSeed/birthPowerNumber) → เคมี wuXing + สอดคล้อง 5 ด้าน (analyzeCoherence
  2 parts มุมผู้ใช้เป็นศูนย์กลาง — convention เดียวกับองค์รวม) + **ภพปัตนิเช็คไขว้** (seventhSign
  เป็น involution → ตรงทางเดียว=ตรงสองทางเสมอ รายงานค่าเดียว) + ธาตุชื่อเขา (ชั้นเสริม
  NAME_TABLE_CAVEAT) · **รูปถ่ายบุคคลที่สาม❌ตามประเมิน** (UI บอกเหตุผลตรงๆ) ·
  API mode "match" — สิทธิ์/เรทร่วมกับคำทำนาย (bucket logic:17 ฟรีครั้งแรก → 20cr) + narrator
  LALA_MATCH_SYSTEM (บุคคลที่สาม=พลังงานเท่านั้น ห้ามตัดสิน/ฟันธง · Productive Clash ใช้เฉพาะ
  เมื่อข้อมูลระบุ — ขันหลังเจอ AI ใช้คำหลวมตอน E2E) · 🔴 **PARTNER_PRIVACY_NOTE: ข้อมูลอีกฝ่าย
  ไม่จัดเก็บ** — rememberEvent เก็บแค่คะแนนรวม (E2E ยืนยัน: history ไม่มีวันเกิด/ชื่อเขา) ·
  UI ส่วน "💕 เช็คกับคนที่คุณสนใจ" ใต้ฟอร์มหลัก (วันเกิดบังคับ · เวลา+จังหวัด/ชื่อ ไม่บังคับ) →
  ตาราง 5 ด้าน + เคมี + ปัตนิ + คำเรียบเรียง AI + caveat 6 ตัว · **E2E จริง:** anon 401 ·
  ฟรีครั้งแรก 200 (AI 6.4 วิ เลขตัวตน 28 ตรงมือ · ปัตนิ กันย์→มีน≠กันย์ ถูก) · ครั้ง 2 → 429

### ✅ ตัวเลือกรูปลักษณ์ภาพเนื้อคู่ "แนวดารา" แบบปลอดภัย — 23 ส.ค. 2569 (ผู้ใช้ขอ)

- **ผู้ใช้ขอ "ใช้ดารา/คนดังเป็นแนวทางอ้างอิง"** → ทำเวอร์ชันปลอดภัย: **ลุคเป็นหมวดสไตล์
  ห้ามอ้างชื่อบุคคลจริง** (สิทธิภาพลักษณ์เชิงพาณิชย์ + ขัดป้าย "ไม่ใช่บุคคลจริง" + นโยบายโมเดลภาพ)
  และเฟรมเป็น "ตัวเลือกการวาดภาพตามความชอบ" ไม่ใช่คำทำนาย (รูปลักษณ์คือหัวข้อที่ตำรายังไม่มี —
  SOULMATE_LOOK_NOTE บังคับแสดงคู่ตัวเลือก)
- soulmate.ts: `LOOK_STYLES` (ธรรมชาติ/ลุคดาราไทย/ซีรีส์เกาหลี/อินเตอร์) + `FACE_STYLES`
  (อบอุ่น/หวาน/คมเข้ม/สดใส) + `AGE_STYLES` (20s-50s ผู้ใหญ่เท่านั้น) — **preset enum เท่านั้น
  ไม่มีช่องพิมพ์อิสระ** ค่านอก enum ถูกเพิกเฉย (เทสต์ล็อก: ยัดชื่อดาราเข้า opts = prompt
  เหมือนไม่ได้เลือกเลย — กัน prompt injection ชื่อบุคคลจริงเชิงโครงสร้าง · เทสต์ 503)
- UI: dropdown 3 ตัวก่อนปุ่มสร้างภาพ + โน้ตกำกับ · API ส่งผ่าน look/face/age เข้า prompt builder
- **verify FLUX จริง:** thai_star+sharp+30s → ภาพลุคพระเอกละครไทย โกรกผมเรียบร้อย คมเข้ม
  วัย ~30 สว่างธรรมชาติ (ดูดีขึ้นชัดตามเป้า "น่าประทับใจ") · caption/share ทำงานเดิมครบ

### ✅ เนื้อคู่: สัญชาติ/สไตล์ลุค + แชร์ FB/สตอรี่ IG + "เนื้อคู่ตามดวงของฉัน" — 23 ส.ค. 2569

- **ตัวเลือกลุคปรับตามผู้ใช้เคาะ:** ถอดโครงหน้า/วัยออกจาก UI (ให้ AI จัดเอง — engine ยังรับ
  param เผื่ออนาคต) เหลือ dropdown เดียว **สัญชาติ/สไตล์ลุค 8 แบบ**: ไทย(default)/เกาหลี/ญี่ปุ่น/
  จีน/ฝรั่ง/อาหรับ/อินเดีย/ลูกครึ่ง (LOOK_STYLES ใหม่ — GENDER_PHRASE เลิกผูก Thai ใน base ·
  ค่านอก enum ตกเป็นไทย ไม่มีทาง free-text เข้า prompt) · ฝรั่งรอบแรกยังออกเอเชีย → ขยับเป็น
  "Caucasian European appearance" แล้วยิงจริงได้ฝรั่งชัด
- **แชร์ (ผู้ใช้ขอ):** ปุ่ม 📘 Facebook (sharer + URL prod ตายตัว — localhost ใช้ไม่ได้อยู่แล้ว) +
  📱 สตอรี่ IG ดาวน์โหลด `/sm/<token>/story` (1080×1920 แพทเทิร์น face-card · Content-Disposition
  attachment) + next.config trace ฟอนต์ · **เปลี่ยนคำทุกจุด: "เนื้อคู่ในจินตนาการของฉัน" →
  "เนื้อคู่ตามดวงของฉัน"** (OG/story/หน้าแชร์/title/native share — ป้าย "ภาพจินตนาการจาก AI"
  ยังอยู่ครบ)
- 🐛 **บทเรียน satori เพิ่ม:** relation_th ของ wuXing ครอบคำด้วย **★ (U+2605) ซึ่งไม่ใช่ emoji**
  → satori ใช้ฟอนต์ตรงๆ = กล่อง tofu ในสตอรี่ (เจอจริง) — `stripCjkForSatori()` ใน
  app/sm/[token]/shared.ts ตัด ★☆+อักษรจีน (通關/印) ก่อนวาดทุกครั้ง (หน้าเว็บจริงไม่ตัด —
  ฟอนต์ระบบมี glyph) · ระวังจุดวาด satori อื่นที่รับ relation_th ไปแสดงในอนาคต
- **verify:** FLUX จริง 3 รอบ (ลุคดาราไทยเดิม→ตะวันตก×2) + E2E สตอรี่ 200 attachment ·
  หัวเรื่องใหม่ขึ้น/คำเก่าไม่เหลือ · เทสต์ 503

### ✅ นรลักษณ์กายา 5 ธาตุ (ค.1) เข้าโหมดเนื้อคู่ + ผิวละเอียด — 23 ส.ค. 2569 (ผู้ใช้ถาม "โหงวเฮ้ง+ธาตุ")

- **🔴 ข้อค้นพบสำคัญ: ตำราจตุพลวัตรมีตาราง ค.1 "นรลักษณ์กายา 5 ธาตุ" อยู่แล้ว** (ภาคผนวก ค
  ที่เตรียมไว้สำหรับ Logic 5/6) — ธาตุ→ลักษณะใบหน้า/รูปร่าง/นิสัย ครบ 5 ธาตุ = หลักโหงวเฮ้ง-ธาตุ
  ที่ผู้ใช้ถามพอดี → **หัวข้อ "รูปลักษณ์" ที่เคยปิดรอเจ้าของตำรา เปิดได้ด้วยข้อมูลตำราเอง**
  (SOULMATE_SCOPE_NOTE เหลือ 4 หัวข้อ · เพิ่มคำถามยืนยันเจตนา ค.1 ในเอกสารถึงเจ้าของตำราแทน)
- `PHYSIOGNOMY_BY_ELEMENT` ใน soulmate.ts (ค.1 คำต่อคำ — เทสต์ล็อก) → readings ทั้งสองโหมด
  แนบ `appearance` + APPEARANCE_CAVEAT ("แนวโน้มกว้างๆ ไม่ใช่คำระบุตัวบุคคล") · narrator เปิด
  พูดรูปลักษณ์**เฉพาะจากข้อมูล ค.1 ที่ให้** ต้องเรียกว่า "แนวโน้ม" · caption ภาพ 1 + แถวผลบนหน้า
- **prompt ภาพ**: รูปหน้า/รูปร่างตามธาตุคู่ (ค.1 แปลงเป็นภาษาภาพเชิงบวก — ธาตุน้ำ "ท้วม มีเนื้อ"
  → soft round friendly) ป้อนอัตโนมัติจากดวง (ไม่ใช่ตัวเลือกผู้ใช้) + **ผิวละเอียด/รูขุมขน**
  (ผู้ใช้ขอ: visible pores + subtle imperfections) · verify FLUX จริง: ธาตุดิน → หน้าเหลี่ยมเต็ม
...
### ✅ แก้เพศหลุดในคอลลาจเนื้อคู่ (1 ก.ย. 2569 — ผู้ทดลองกลุ่มแรกรายงานพร้อม screenshot)

**อาการ:** เลือกเพศหญิง แต่ 1 ใน 4 ช่องออกมาเป็นผู้ชาย — ช่องที่หลุดคือท่า "ยืนกอดอก"
**ต้นเหตุ:** prompt เอ่ยเพศครั้งเดียวตอนต้น แล้วรายการท่าทั้ง 4 เป็นกลางทางเพศ → FLUX drift
**แก้:** GENDER_WORDS (noun/pos/subj) ทอสรรพนามเพศเข้าทุกท่า + ประโยคบังคับ "same gender in
all four panels · no other person" (แพทเทิร์นเดียวกับที่แก้รูปลักษณ์ 25 ส.ค. — ย้ำสิ่งที่ห้าม drift
ให้ถี่) · เทสต์ล็อก: หญิงต้องมี woman ≥3 + her ≥4 + ห้าม \bman\b เดี่ยว (กันเหนี่ยวนำ — regex
ระวัง "woman" มี "man" เป็น substring แต่ \b กันได้) · ชายกลับด้าน (554)
**ยิง FLUX จริง 2 รูปยืนยัน:** หญิงครบทุกช่องรวมท่ากอดอก · variance กริดยังมีบ้าง (รูปแรก
ช่องซ้อน) = อาการเดิมที่รู้จัก ไม่ใช่เรื่องเพศ

### ✅ ภาพเนื้อคู่ตรงแบบที่เลือกรอบสอง (1 ก.ย. 2569 — ผู้ทดลองทัก: เลือก ข ภาพยังหน้ากลม/ตัวหนา)

**ต้นเหตุ:** การแก้เพศหลุด (คอมมิตก่อนหน้า) เพิ่มข้อความเพศ/panel ต่อจากประโยคแรกหลายประโยค
→ วลีรูปลักษณ์ที่เคย "ขึ้นต้น prompt" (fix 25 ส.ค.) ถูกดันลึกจนโมเดลภาพให้น้ำหนักน้อย
**แก้:** วลีรูปลักษณ์**ฝังติดตัวบุคคลในประโยคแรกสุด** ("the same an adult woman who clearly and
unmistakably has <traits>") + ย้ำซ้ำระดับ panel ("In every single panel she has the exact same
<traits>… no panel may deviate") + ท่าเต็มตัว "exactly as described" — เพศยังย้ำทุกท่าเหมือนเดิม
**ยิง FLUX จริง 2 รูป (หญิง×ไม้ เงื่อนไขเคสผู้ใช้):** หน้ายาวโครงชัด+สูงเพรียวครบทุกช่อง
🔴 **บทเรียนซ้อน:** วลีที่ต้อง "ชนะ" ใน prompt FLUX ต้องอยู่ประโยคแรก — การเพิ่ม emphasis เรื่องใหม่
(เพศ) ต้องไม่ดันของเดิม (รูปลักษณ์) หลุดตำแหน่ง · แก้ prompt ครั้งหน้าตรวจลำดับก่อนความยาว

### ✅ ภาพเนื้อคู่อัตโนมัติ + เพศคู่ไม่บังคับเลือก (2 ก.ย. 2569 ผู้ใช้เคาะรอบสอง)

🔴 **บทเรียน deploy fail 3 รอบ (2 ก.ย. 2569):** ต้นเหตุคือ ESLint `react/no-unescaped-entities`
(เครื่องหมาย " ใน JSX text) — **`npm run build` บนเครื่อง dev นี้ผ่านทั้งที่ lint พัง แต่ Vercel
fail ที่ขั้น Linting** (อย่าเชื่อ build local ว่าครอบ lint) → ก่อน push ที่แตะ .tsx ให้รัน
`npx eslint <ไฟล์ที่แก้>` เสมอ · อาการบน GitHub status เห็นแค่ "Deployment has failed" —
log จริงต้องดูใน Vercel dashboard (ผู้ใช้ส่งให้)

- **ไม่มีทางแยก ก/ข = สร้างภาพอัตโนมัติทันทีหลังคำทำนายขึ้น** (useEffect + guard key ยิงครั้งเดียว
  ต่อชุดผล — มีทางแยก = รอคลิกการ์ดซึ่งยิงทันทีอยู่แล้ว) → จากฟอร์มถึงภาพ = ปุ่มเดียว
- **เพศคู่ที่สนใจไม่บังคับแล้ว** — ทับมติ "ห้ามเดา ไม่เลือก=400" (21 ส.ค.): เลือกเอง = ตามนั้น ·
  ไม่เลือก = อนุมานตรงข้ามจากเพศโปรไฟล์ (male↔female) · ไม่มีข้อมูลเพศ = "any" (แบบรวม/โมเดลเลือก)
  — ตรรกะที่ route จุดเดียว (`resolvePartnerGender` inline) มีผลทั้ง reading/images
- ⚠️ auto-gen ใช้สิทธิ์ฟรีรูปแรก/หักเครดิตตามกติกาเดิมโดยไม่มีคลิกแยก — เจตนาของผู้ใช้
  ("เลือกแนวทางแล้วควรสร้างภาพเลย") · เครดิตไม่พอ = ข้อความ 429 เดิมโชว์แทนภาพ ไม่พัง

### ✅ ลดขั้นตอนก่อนได้ภาพเนื้อคู่ (2 ก.ย. 2569 ผู้ใช้ทัก "เลือกหลายขั้นเกินไป")

- **ปุ่มบนการ์ดทางแยก ก/ข = สร้างภาพทันที** ("🎨 สร้างภาพตามแบบ ก เลย") — เดิมแค่ set state
  แล้วต้องไปกดปุ่มสร้างภาพอีกที · generateImages(choice?) รับค่าตรง (state async) + guard
  imgLoading + auto-scroll ไปส่วนภาพ · เลือกแบบใหม่ = ล้างภาพเก่าแล้ววาดใหม่
- เคสมีทางแยก: ปุ่มสร้างภาพหลักหายไป (เหลือข้อความชี้ไปการ์ด — ไม่มีปุ่มซ้ำสองจุด) ·
  เคสไม่มีทางแยก: ปุ่มเดียวเหมือนเดิม (1 คลิก)
- dropdown สไตล์/ลุค/โทนผิว **ยุบเป็น <details> "🎛 ปรับแต่งภาพ (ไม่บังคับ)"** — default
  ทำงานเองหมด ผู้ใช้ไม่รู้สึกว่าเป็นขั้นตอนบังคับ
- verify: browser เรนเดอร์สะอาด · fetch path เดิมที่ E2E ผ่านแล้ว (เปลี่ยนเฉพาะจุดเรียก)

### ✅ ภาพเนื้อคู่ default เป็นสเก็ตช์สีน้ำ + ตัวเลือกสไตล์ภาพ (2 ก.ย. 2569 ผู้ใช้เคาะหลังดูตัวอย่าง)

ผู้ใช้เสนอ "ลองสเก็ตช์/สีน้ำมันให้ดูเป็นจินตนาการกว่า" → ทดลองจริง: **สเก็ตช์+สีน้ำสวยชัด สื่อ
จินตนาการ + หนุนป้าย "ไม่ใช่บุคคลจริง" ในตัว** · สีน้ำมันบน schnell แทบไม่ต่างภาพถ่าย (ไม่เปิด)
→ ผู้ใช้เคาะ: **สเก็ตช์เป็น default ทั้งระบบ** ภาพถ่ายเลือกกลับได้
- `ART_STYLES` (soulmate.ts): sketch (default) / photo — enum เท่านั้น ค่านอกตกเป็น default
  (เทสต์ล็อก injection) · สลับเฉพาะประโยคเปิด+ท้าย prompt — ข้อจำกัดตัวตน (เพศ/ลุค/ผิว/มิดชิด/
  รูปลักษณ์/ท่า) คงเดิมทุกสไตล์ (เทสต์ล็อก) · เพิ่ม "full head never cropped" (จูนจากตัวอย่างแรก
  หัวโดนครอป) + "no artist signature" (สไตล์วาดมือชอบเซ็นชื่อ — เจอจริง)
- dropdown "สไตล์ภาพ" หน้า /soulmate + SoulmateBody.artStyle · เทสต์ 559
- 🔴 **regression ที่จับได้พ่วง: บรรทัด `skin:` หลุดจาก call collage ตอน rewrite 084a8df** —
  prefSkin ที่ผู้ใช้เลือกไม่เคยถึง prompt (เงียบ) · คืนแล้ว — บทเรียน: rewrite call site ต้องเทียบ
  พารามิเตอร์ครบทุกตัวกับของเดิม
- FLUX verify 3 รูป (หญิงไม้ไทยผิวสองสี ×1 · ชายน้ำไทย ×1 + รอบ signature): หัวครบ มิดชิด
  ตรงแบบ ไม่มีลายเซ็น

### ✅ /logo ปรับ 2 จุด + ภาพเนื้อคู่ย้ายเป็น FLUX dev + สไตล์สีน้ำมัน (9 ก.ย. 2569 ผู้ใช้สั่ง)

1. **ช่องกรอก /logo อ่านไม่ออก** — กับดักเดิม (var(--text) บนธีมลายพื้น = ครีมกลืนช่อง) → สี/พื้นช่อง
   ตายตัว #1d1812/#fffdf8 · verify browser: computed rgb(29,24,18)
2. **AI ตีความชื่อแบรนด์ → brief ภาพ** (`lib/logo/brief.ts` + `/api/logo/brief` · 2 เทสต์ → รวม 574):
   ผู้ใช้ทักว่าโลโก้ที่ไม่พิมพ์อธิบายออกมาลอยๆ → Haiku (role router ~฿0.03) ตีความชื่อเป็น
   {industry, product, motifs≤3, mood, summaryTh} · **เส้นแบ่ง §16 คงเดิม**: AI ทำแค่ "ความหมายของชื่อ"
   ธาตุ/สไตล์/สียังมาจาก engine · `sanitizeBrief` กัน injection (เทสต์ล็อกอักขระ/ความยาว) ·
   ปุ่ม "✨ ให้ AI ตีความชื่อ เติมให้" เติมช่องความต้องการ (แก้ได้) · **ไม่พิมพ์ = route เติมให้อัตโนมัติ**
   AI ล้ม = prompt เดิม · ล็อกอินเท่านั้น (anon 401) · E2E จริง: "Lala Coffee"→coffee cup/latte art ·
   "รุ่งเรืองยางยนต์"→tire/sunrise/wheel · "สมชาย"→บอกตรงว่าเป็นชื่อบุคคล เดาธุรกิจท้องถิ่น
3. **ภาพเนื้อคู่ → FLUX dev** (`FAL_MODELS.soulmate`): A/B prompt สเก็ตช์เดียวกัน schnell/dev/pro 1.1 —
   ผู้ใช้เลือกรูป dev · dev ≈ ฿0.71/รูป → 30cr = 10× ผ่านกฎ (pro ≈ ฿1.44 → 5.3× หลุดกฎ ≥6× ต้องขึ้น 40cr
   จึงไม่เลือก) · pricing costThb 0.66→0.75 · ⚠️ dev ยังมี grid variance บ้าง (บางรอบ 3 ช่อง) เท่า schnell
4. **สไตล์ "สีน้ำมันคลาสสิก" เพิ่มเป็นตัวเลือกที่ 3** (ART_STYLES.oil) — บน dev ออกมาเป็นจิตรกรรมจริง
   (ต่างจากผลทดลองบน schnell 2 ก.ย. ที่เหมือนภาพถ่าย)
5. **default เปลี่ยนเป็น "จิตรกรรมสีไม้" (ART_STYLES.pencil — ผู้ใช้เคาะ 9 ก.ย. 2569 "สีไม้โอเคเลย"
   หลังดูตัวอย่าง 2 รูปบน dev):** หน้าคม ผิวนุ่ม เห็นเนื้อกระดาษ · ไม่มีลายเซ็นหลุด (tail ยังมี
   "no artist signature") · สเก็ตช์สีน้ำ/สีน้ำมัน/ภาพถ่ายยังเลือกกลับได้ใน "🎛 ปรับแต่งภาพ" ·
...
### ✅ ขั้นจัดโครงคำตอบ (answer structurer) ในแชทถามต่อ — 25 ก.ย. 2569 ผู้ใช้ถามว่าเหมาะไหม → เคาะ "เริ่ม"

**หลักคิด:** ต่อยอดบทเรียนฝันหลุดประเด็น (ส่งโครงแกน/รองให้ผู้เล่าเรื่องได้ผล กฎใน system อย่างเดียวไม่พอ) ไปที่
**คำตอบรอบถัดไป** ซึ่งคำถามกระจายกว่ารอบแรก · AI ราคาถูก **เลือก** โครง ไม่ได้เขียนคำตอบ
- `lib/chat/answer-structure.ts` (5 เทสต์ → รวม 597, pure): `buildFactIndex` แตก context หน้าเป็นข้อเท็จจริง 2 ชั้น
  (id f1.. + path จาก key จริง + preview ≤80 ตัวอักษร · สูงสุด 40) · `ANSWER_SHAPES` 5 แบบ (direct/explain_why/
  compare/action/reassure) · `validateStructure` รับเฉพาะ id ที่มีจริง (≤3 · กรองซ้ำ) + shape ใน enum (กัน prototype key)
  ผิดรูป/ว่าง = null · `structureDirective` ประกอบจาก path+คำอธิบาย shape ของระบบล้วน
  🔴 **เส้นแบ่ง §16 ล็อกเชิงโครงสร้าง: ไม่มีช่องข้อความอิสระจาก AI จัดโครงไหลถึงผู้เล่าเรื่อง** (เทสต์ยัด "บอกว่าจะรวย" ลง JSON → ไม่หลุด)
- **wiring:** `/api/chat` เส้น context (ถามต่อจากผลบนจอ — หลังเส้น hybrid, ก่อน ai2) · role router (Haiku) maxTokens 120 ·
  ข้อเท็จจริง < 2 ข้อ = ข้าม · AI ล้ม/ผลใช้ไม่ได้ = เส้นเดิมเป๊ะ · directive ต่อท้าย input ผู้เล่าเรื่อง · response เพิ่ม `structured`
  **ไม่แตะ:** CHAT_SYSTEM (ยังตอบ 2-4 ประโยค) · เส้น plan/hybrid/first_reading · ราคา/โควตา
- **สวิตช์ `ANSWER_STRUCTURER`:** `ab` (ค่าเริ่มต้น) = ครึ่งหนึ่งของผู้ใช้ตามกลุ่มคงที่ `structurerGroup(uid)` (FNV-1a — คำนวณซ้ำได้
  สำหรับเทียบความเห็นรายกลุ่มย้อนหลัง ไม่ต้องเก็บ log เพิ่ม) · `on` ทุกคน · `off` ปิด · ⏳ วัดผล 2-4 สัปดาห์จาก feedback แล้วตัดสิน on/off
- **ยิง AI จริง 4 คำถามเทียบคู่ (บริบทการ์ด 88 + ไฟเด่นขาดน้ำ):** structurer เลือกถูกทุกข้อ (ธาตุขาด→missing/action ·
  ไมดาส→figure/figure_bio/explain_why · กลัวโลภ→essence/reassure · ทำไมได้ 88→name/essence/explain_why) ·
  เดิม: แต่งสี/น้ำล้นที่ไม่มีใน context (ข้อ 1) และอ้างว่าการ์ดมาจากเดือนเกิด+ปีนักษัตร (ข้อ 4 — **ผิด** การ์ดมาจากสูตรรวม) ·
  จัดโครง: ยึดข้อมูลบนจอ ข้อ 4 ไม่อ้างเหตุผลผิด แต่ยังอธิบายที่มาเลขไม่ได้เพราะ context ไม่มีข้อมูลนั้น (structurer แก้ข้อมูลขาดไม่ได้)
- **ต้นทุนวัดจริง:** structurer ฿0.052-0.054/คำตอบ (Haiku in ~1,310/out ~33) · ความหน่วง 0.85-1.25 วิ · คำตอบ context รวม ~฿0.19
  ยังต่ำกว่า costThb chat_question 0.35 เดิม → **ไม่ต้องขยับราคา** (กำไร ≥13× ที่ floor)
- ⚠️ **ทำนายฝันยังไม่มีเส้นถามต่อ** — หน้า /dream ส่งทุกข้อความเป็นฝันใหม่ (หัก 20cr) และแชทลอยซ่อนบน /dream · การเปิดเส้นถามต่อของฝัน
  = ฟีเจอร์ใหม่ (UX+ราคา) ต้องให้ผู้ใช้เคาะก่อน

### ✅ ทำนายฝัน "จับใจความ" — แกนเรื่องของฝัน (10 ก.ย. 2569 ผู้ใช้รายงาน "คำทำนายหลุดประเด็นเล็กน้อย" → ตรวจ → อนุมัติแก้)

**การตรวจ (engine 8 ฝัน + AI จริง 3 ฝัน ใช้ context ชุดเดียวกับ prod):** ต้นตออยู่ที่ **engine ส่งบริบทให้ AI ผิดน้ำหนัก**
ไม่ใช่ตัว AI — ① `rankAndDedupeSymbols` เรียง "ชื่อยาวก่อน" → ฝัน "แม่ที่เสียไปแล้วมาหา…วิ่งตามไม่ทัน" ส่ง บ้าน/ประตู/แม่/ไม่ทัน/วิ่ง/
**ให้** (จับผิดจาก "ยิ้มให้")/ยิ้ม/เดิน น้ำหนักเท่ากัน → AI สรุป "กลัวโอกาสหลุดลอย" + ถามปิดท้ายเรื่อง "ทำไม่ทันเวลา"
② คำสั้นซ้อนในคำยาว: "แฟนเก่า" จับทั้ง แฟนเก่า + **แฟน/คนรักปัจจุบัน** → AI พูดถึง "ความหึงหวงคนรักปัจจุบัน" ในฝันที่ไม่มี ·
"แม่น้ำ" 2 แถวส่งซ้ำ ③ ธีมจิตวิทยาชื่อมีวงเล็บ "ลิฟต์ (ขึ้น/ลง/ติด)" แตกเป็น variant "ลง" → จับ "รถพุ่งลงแม่น้ำ" เป็นธีมลิฟต์
(และ "เงิน (ได้เงิน/…)" ไม่เคยจับได้เลยเพราะ variant แรกเป็น "เงิน (ได้เงิน" → ฝันเรื่องเงินหลุดไปปลุก AI-1 ฿7)
④ context ไม่มีสัญญาณ "แกนเรื่อง" — โครงคำตอบไล่สัญลักษณ์เป็นรายการ AI จึงสรุปจากตัวที่ความหมาย "แรง" สุด

**ที่แก้ (`lib/engine/dream-match.ts` + `dream.ts` + `app/api/dream/route.ts` · เทสต์ใหม่ `tests/dream-focus.test.ts` 4 ตัว → รวม 592):**
- `rankMatches` รู้ตำแหน่ง: เรียง **ตามตำแหน่งที่ผู้ใช้เล่า** (คำนาม+กริยาสำคัญปนกันตามฉาก) · ตัดตัวที่ช่วงคำซ้อนอยู่ในตัวยาวกว่า
  (แฟน⊂แฟนเก่า · ฟัน⊂ฟันหลุด — เทสต์เดิม "ฝันว่าฟันหลุด" ปรับ expectation เป็นตัวเจาะจง) · ช่วงเท่ากันเก็บตัวแรกในฐาน (แม่น้ำ ×2) ·
  กริยาทั่วไป `LOW_SALIENCE` (เดิน/ยิ้ม/หัวเราะ/กิน/ดื่ม/ให้/นั่ง/ยืน/นอน) ต่อท้ายไม่เกิน 2 · กริยาอื่นไม่เกิน 4 เมื่อมีคำนาม ≥ 2 · เพดาน 8 เท่าเดิม
- `coreSymbols` = 2 ตัวแรกตามที่เล่า (ไม่นับกริยาทั่วไป) → `result.core_symbols` **เฉพาะเส้นตัดคำ** (เส้น parity Python ไม่มีฟิลด์นี้ — golden ไม่กระทบ)
  🔴 ทดลองแล้ว "ยึดคำนามอย่างเดียว" ผิด: ฝันเรื่องสอบ AI ไปเล่า "บันได" — แกนต้องเป็น "สอบ" แม้เป็นหมวดการกระทำ
- TRAP "ให้": ยิ้มให้/ทำให้/พูดให้/บอกให้/เพื่อให้/ปล่อยให้/อยากให้ ไม่ใช่ "ให้ / บริจาค"
- `themeVariants`: ส่วนหน้าวงเล็บใช้ทุก variant · ในวงเล็บเฉพาะวลี ≥ 2 คำ (หาไม่เจอ/วิ่งไม่ถึง/ได้เงิน) + allow "ผีอำ" (ICU มองคำเดียว) ·
  stop "ในฝัน" · reverse-check `v.includes(dreamText)` เฉพาะข้อความ ≤ 10 รูปอักขระ (เดิมข้อความยาวก็เทียบกลับ)
  ผลข้างเคียงที่รับรู้: ธีม "วิ่ง" และ "เงิน" กลับมาจับได้ตามชื่อจริง (เดิมไม่เคยจับเพราะบั๊กวงเล็บ)
- route: context เพิ่ม `แกนเรื่อง` + ทุกสัญลักษณ์ติดป้าย `บทบาท: แกนเรื่อง | รอง (รายละเอียดฉาก)` + **บรรทัดสั่งงานท้าย input** ระบุชื่อแกน/รอง
  ("ห้ามยกความหมายของสัญลักษณ์รองเป็นข้อสรุปหรือคำถามปิดท้าย") · กฎ narrator 5.1 + โครงคำตอบ 🗝️/🔮 ยึดแกนเรื่อง
  🔴 บทเรียน: ใส่กฎใน system อย่างเดียว Flash ยังหยิบ "ไม่ทัน" มาปิดท้าย — ต้องติดป้ายในตัวข้อมูล + ย้ำใน user turn ถึงหาย
- **verify AI จริงหลังแก้:** แม่ที่เสียไป → เปิดด้วยแม่+บ้าน ปิดท้ายถาม "คำสอนของแม่" ✓ · แฟนเก่า → ไม่มีคนรักปัจจุบันโผล่ ถามว่ารู้สึกยินดีหรือกังวล ✓ ·
  สอบไม่ทัน → แกน "สอบ/ไม่ทัน" บันไดเป็นรายละเอียด ✓ · แมวกัดงู → แกน แมว/บ้าน เล่างูเป็นเหตุการณ์ในฉาก ✓
- ⏳ **ช่องว่างฐานที่พบ (รอชุดข้อมูลตำรา ห้ามแต่ง):** ไม่มี เงิน/ตึก/กระเป๋า/ใบไม้/รถ(คำนาม) เป็นสัญลักษณ์ — ฝันเรื่องเงินตอนนี้จับได้แค่ธีมจิตวิทยา
...
- **ที่มา:** ผู้ใช้รายงาน "กดดูคำทำนายแล้วไม่สร้างคำทำนาย" → ตรวจ prod จริง (E2E 7 แบบ + browser) ระบบทำงานปกติ
  ต้นเหตุคือบัญชี inowok.th ใช้สิทธิ์ฟรีครบแล้ว+เครดิต 0 → 429 แสดงเป็นบรรทัดแดงเล็กเหนือปุ่ม ดูเหมือนไม่มีผล
  (ข้อเสนอแก้ UX กำแพง 429/401 ให้เด่น **ยังไม่อนุมัติ** — ไม่ได้ทำ ตามกติกา §0.0)
- **UI /soulmate ตัดออก:** 💗 สเปกที่ชอบ (รูปร่าง/โครงหน้า/บุคลิก) · 🎛 ลุคสัญชาติ/โทนผิว → เหลือ select "สไตล์ภาพ"
  ตัวเดียวก่อนสร้างภาพ · **engine/API ยังรับ prefBody/prefFace/prefPersona/prefSkin/look เหมือนเดิม** (ไม่ส่ง = ลุคไทย
  default ไม่มี preference layer · dual path ยังขึ้นอัตโนมัติจาก rankedElements) — ถอดเฉพาะหน้าจอ ไม่แตะกติกา Mirror
- **ART_STYLES ใหม่ (ลำดับ = dropdown):** sketch สเก็ตช์สีน้ำ · pencil สีไม้ (default คงเดิม) · **expressionist
  สีน้ำมันเอ็กซ์เพรสชันนิสม์ (แทน oil คลาสสิก — key "oil" ถอด client เก่าส่งมาตกเป็น default เทสต์ล็อก)** · photo ภาพสมจริง ·
  prompt expressionist บังคับ "the face stays clearly recognizable and well-proportioned" กันฝีแปรงกลืนนรลักษณ์ ค.1 ·
  **ทดลอง FLUX dev 3 รอบ:** ได้ "จิตรกรรมพื้นสีจัด ฝีแปรงที่พื้นหลัง ใบหน้ายังเนียน" (dev ดึงเข้าสมจริงเพราะ
  ประโยคตัวตนหนัก) · ถ้อยคำเข้มกว่านี้ไม่เพิ่มฝีแปรงบนหน้าแต่ทำเสื้อหลุดมิดชิด 1 ช่อง → คงเวอร์ชันกลาง (ผู้ใช้ควรดู
  ตัวอย่างจริงก่อนตัดสินว่าพอไหม) · พ่วง: **collage prompt ไม่เคยมี "no artist signature"** (มีเฉพาะ prompt legacy) —
  ตัวอย่างรอบแรกมีลายเซ็นหลุดมุมล่าง → เติมแล้ว (อยู่ในขอบเขตงานสไตล์วาดมือ)
- 🐛 **พ่วงที่จับได้ตอนอุ่น OG หลัง deploy: รูป OG หน้าแรก 500 บน prod** (การ์ด 200 · local ปกติ) — ต้นเหตุ
  `app/opengraph-image.tsx` อ่าน `public/mascot.png` ด้วย fs แต่ `route.js.nft.json` trace แค่ฟอนต์ ไม่รวม public/
  (Vercel lambda ไม่มีไฟล์ → ENOENT) · แก้ที่ next.config `outputFileTracingIncludes["/opengraph-image"]` เพิ่ม
  `./public/mascot.png` · **บทเรียน: ทุก route ที่ readFile ไฟล์ใน public/ ต้องประกาศใน tracing includes เสมอ**
  (แก้นอกขอบเขตคำสั่งเพราะทำให้แชร์หน้าแรกพัง — รายงานให้ผู้ใช้ทราบแล้ว)
- **ปุ่ม:** หลัง `res.reply` ขึ้น ปุ่มหลักเปลี่ยนเป็น "⬇️ ดูคำทำนายด้านล่าง" (type=button เลื่อนไป `#soulmate-reading`
  ไม่ยิงซ้ำ) + ปุ่มรอง "🔁 คำนวณใหม่" (submit ตามกติกาเครดิตเดิม) · ระหว่างโหลดยัง "กำลังคำนวณ..."
...
- **梅花易数 verify กับต้นฉบับ (wikisource 卷一/卷二) ครบ**: สูตรตั้งกัว (ปี地支+เดือน+วันจันทรคติ mod 8 ·
  +时辰 mod 8 · รวม mod 6 · 乾兌離震巽坎艮坤) · ตาราง 8 กัว ทิศ後天+地理 (ตรงตารางเอกสาร B ทุกทิศ) ·
  失物占: 用=ของหาย · **「以變卦為失物之所在」** · 體克用=หาได้ช้า 用生體=ง่าย 比和=ไม่หายจริง
  體生用=ยาก 用克體=不可尋 → ⚠️ ค่าหลังต้อง**แปลงถ้อยคำ** (กติกาแมวหายห้ามบอก "หาไม่ได้")
  · **บล็อกเกอร์: ต้องสร้างปฏิทินจันทรคติจีน** (定朔: จันทร์ดับ+中氣+อธิกมาส — เครื่องมือดาราศาสตร์
  มีครบ verify กับวันตรุษจีน 2026-2035 ได้) + เลือกธรรมเนียม 子時ข้ามวัน/เลขปีตาม立春 · ประมาณ 1 วัน
  → **ข้อเสนอ: รอบ implement แยกเมื่อผู้ใช้เคาะ** ใส่เป็นชั้น T น้ำหนักต่ำ ให้ calibration ตัดสิน
- **ยามสามตา มีต้นฉบับเปิด** (thaiboraan + ศิลปวัฒนธรรม — เอกสาร B เข้าใจว่าไม่มี): สามตำแหน่ง
  ใส/ปลอด/ดำ นับตามข้างขึ้น 1-2-3 / ข้างแรม 1-3-2 ตามวันค่ำ+ยาม 1.5 ชม. · ของหาย: เศษ 1 คนในเรือน ·
  2 แกนเหนือ-ใต้ · 3 แกนตะวันออก-ตะวันตก · สัตว์หาย: ดำ=ได้ ใส=ได้ช้า **ปลอด=ไม่ได้** →
  **ไม่แนะนำเป็นชั้นคะแนน** (ทิศเป็นแกนไม่ใช่ 8 ทิศ + คำทำนายชี้ขาดชนกติกา/safety) — อย่างมากบรรทัด
  ภาษาตำราที่กรองแล้ว น้ำหนัก 0

### ✅ ตามหาแมวหาย เฟส 1 — แผนลำดับการค้น (8 ก.ย. 2569 ผู้ใช้ส่งเอกสารออกแบบ 2 ไฟล์ + "เริ่มตามความเหมาะสม")

**ที่มา:** ผู้ใช้ส่ง `ตำราดูดวงแมว_งานวิจัยฐานความรู้.md.pdf` + `_โมดูลแมวหาย_ตรรกะการคำนวณ.md.pdf`
(ร่าง 8 ก.ย. 2569 — สอดคล้องกับ /cat และข้อเสนอเดิมเกือบทั้งหมด) → ประเมินแล้วรับแบบ "ตัดให้เล็ก":
- **หลักคิดที่ล็อก:** ไม่มีศาสตร์ใดทายตำแหน่งแมวหายได้ — คำนวณได้แค่ "ลำดับการค้นที่คุ้มค่าที่สุด" ·
  ทุกช่อง > 0 ห้ามตัดพื้นที่ · **ฟรี ฿0 ไม่ต้องล็อกอิน ไม่ใช้ AI** (คนร้อนใจไม่ควรเจอกำแพง)
- `lib/engine/lost-cat.ts` (5 เทสต์ → รวม 572): **ชั้น B สถิติ verify กับเปเปอร์ฉบับเต็ม (PMC5789300)**
  Huang 2018 n=1,210: ในบ้านล้วนมัธยฐาน 39 ม./75% ≤137 ม. · ออกนอกบ้าน 300 ม. · พบ 34/50/61% ใน
  7/30/365 วัน · 83% นอกอาคาร (สวนคนอื่น 20% รอหน้าบ้าน 19% ใต้พุ่มไม้ 16% ใต้บ้าน 10%) · ค้นกลางคืน
  ไฟฉายช่วยสุด · ขี้สงสัยพบในบ้านเพื่อนบ้าน 17% vs 6% — **น้ำหนักวงระยะ 4 วงเป็นการแปลงจากการกระจาย
  (ออกแบบเอง ประกาศใน LOST_CAT_STATS_NOTE)** · ชั้น U ภูมิประเทศจากผู้ใช้กำหนดทิศ (ทางออก +3 ·
  ที่กำบัง/เคยเห็น +1.5 · ถนน/เสียง −1 floor 0.2 · บ้านเก่า +1 และดันวงไกล ×1.5) · ชั้น T ธาตุแมว→ทิศ
  +0.5 (⚠ ออกแบบเสริม ELEMENT_DIR_NOTE — เทสต์ล็อกห้ามชนะทางออก) · fusion ย่อ: score = P_U[dir]×P_B[ring]
  → 32 ช่อง top-5 พร้อมเหตุผลสถิติ+จุดที่ควรดู · ฤกษ์ออกค้น = ช่วงแมวเคลื่อนไหว (05-07/18-22/ดึก) +
  bestTimeToday อุบากอง (บอกว่าทับซ้อนไหม) · lure ทิศธาตุ+สีผ้า (ภาษาตำรา) · hope line + checklist 7 ข้อ
  · **fixture ตามเอกสาร: ในบ้าน+ขี้กลัว+2 วัน → top-1 = วง 0-50 เสมอ** · regex ล็อกห้ามฟันธง/บอกเลิกค้น
- **ตัดออก/รอรอบวิจัยแยก (ผู้ใช้เคาะ "แล้ววิจัยแยกต่อ"):** 梅花易数 ตั้งกัวจากเวลาหาย (ต้องมีปฏิทิน
  จันทรคติจีน + 8 กัว + 體用 — ขนาดเท่ารอบ Jyotish) · ยามสามตา (ไม่มีแหล่งเปิดสูตร) · heatmap/แผนที่ ·
  ชั้น "แมวให้โทษ = สัญญาณสุขภาพ" (ผมตัดสินไม่เปิด — ชนกฎห้ามแตะสุขภาพสัตว์)
- **calibration loop (ข้อเสนอที่ดีที่สุดในเอกสาร):** `migration 046` (รัน prod แล้ว) `lost_cat_cases_e`
  (inputs+predicted top-5, auth_uid nullable) + `lost_cat_outcomes_e` (found/direction/ring/place/days —
  ตัวเลือกเดียวกับสถิติ Huang) RLS ไม่มี policy · `/api/lost-cat` mode plan (คำนวณ+บันทึกเคส anon ได้ ·
  บันทึกพังยังคืนแผน) / mode outcome (validate enum + uuid) → ต่อไปวัด hit-rate top-3 แยกชั้น B/U/T แล้ว
  ปรับน้ำหนักจากของจริง — ถ้าชั้นตำราไม่ต่างจากสุ่ม ลดน้ำหนักตามจริง
- หน้า `/cat/lost` (marble · ชิป 8 ทิศ multi/single · ผล 5 ลำดับ+ช่วงออกค้น+checklist+caveat+ฟอร์ม
  "เจอแล้ว") · ลิงก์จาก /cat header · sitemap 0.7 · **verify:** API E2E (plan anon → caseId · top ใต้ r0
  ตรง engine · outcome บันทึก · uuid ปลอม 400 · cascade cleanup) · browser จริง
- **ไฟล์ A ที่รับมาใช้:** ที่มาต้นฉบับเพิ่ม (British Library Or.16797/Or.16008 · Clutterbuck 1998/2004 ·
  TU Digital dc:141793 · "มัจฉรานีติ" 26/12 อีกระบบ) + **จุดที่แหล่งขัดกัน** (วิลาศ: ดำแต้มขาว 8 แห่ง vs
  แถบขาวอก-ท้อง-หน้า · รัตนกำพล: สีสังข์ vs ขาว · กรอบแว่นบางฉบับมีปานอานม้า) — ระบบยึดฉบับ
  วัดอนงคารามเป็น canonical ตามที่เอกสารแนะ · โมดูลถัดไปในเอกสาร (ลางแมว 10 ข้อ · มาเนกิเนโกะ ·
  สีขน↔นิสัย UC Davis) ยังไม่ทำ ต้องติดป้ายชั้นหลักฐานทุกจุด · ตัวเลข "จดทะเบียน 2567/มรดก 2561"
  เอกสารเองบอกยังไม่ verify ห้ามใส่ผลิตภัณฑ์

### ✅ ดูดวงแมว เฟส 1 — ตำราแมวศุภลักษณ์ + ชั้นธาตุ (6 ก.ย. 2569 ผู้ใช้สั่ง "เริ่มตามความเหมาะสม")

**การตัดสินใจ (ผู้ใช้มอบให้เลือก):** ① เล่าเฉพาะด้านมงคล — **ไม่จำแนกแมวเป็นแมวร้าย/ให้โทษเด็ดขาด**
(ตำรามีหมวดแมวให้โทษ 6 ชื่อแต่ไม่มีแหล่งไหนให้ลักษณะละเอียด + เสี่ยงทำร้ายใจ/ทิ้งแมว — เทสต์ regex ล็อก
ทุก string เหมือนกรอบ maraka) ② เริ่มเฟส 1 ฟอร์มเลือกลักษณะ (เฟส 2 รูปถ่าย+vision · เฟส 3 ผูกองค์รวม/
ชื่อแมว/ฤกษ์รับแมว/หน้า SEO รายชนิด — ยังไม่ทำ)
- **งานวิจัย (WebSearch/WebFetch cross-check):** รายชื่อ 17 ชนิดตรงกัน 4 แหล่ง (silpa-mag · mthai ·
  lib.ru.ac.th · th.wikipedia) · ลักษณะรายชนิดจากตำราวัดอนงคาราม (catdumb) cross-check กับ lib.ru.ac.th
  ใน 6 ชนิดที่ยังเหลือปัจจุบัน ตรงกัน · คุณรายชนิดจาก mthai 371168 · 🔴 **ขาวมณีไม่อยู่ในสมุดข่อย**
  (แมวไทยยุคหลัง) แหล่งต่างกันนับ/ไม่นับ → ใส่พร้อมป้าย inTamra=false
- `lib/engine/cat-tamra.ts` (5 เทสต์ → รวม 567): CAT_TAMRA 17+1 (ลักษณะ/ตา/คุณ คำต่อคำจากแหล่ง) ·
  enum ที่เจ้าของสังเกตเองได้ (CAT_COATS 11 · CAT_MARKS 13 · CAT_EYES 6 — ค่านอก enum เพิกเฉย) ·
  `matchCatTamra` (กลุ่มดำ-ขาวแยกด้วยแต้ม · ดำล้วนแยกด้วยตา: ตาดำ=นิลรัตน์ อื่น/ไม่รู้=โกญจา · ตาต่างตำรา
  = จับคู่ให้แต่มีโน้ต "ใกล้เคียง") · ชั้นธาตุจากสีขนผ่าน COLOR_TO_ELEMENT (ชั้นเสริม ประกาศ) · เคมี
  wuXingScore มุมเดียวทั้งระบบ + Productive Clash · สีของใช้แมว: พิฆาต=ธาตุสะพาน (bridgeElement) ·
  ชื่อแมว→nameElement (ชั้นเสริม) · นอก 17 ชนิด = "แมวทั่วไป" + อ่านผ่านธาตุ (ไม่ใช่คำตัดสิน)
- `/cat` (marble) engine ฝั่ง client ฿0 ผลขึ้นทันที + ตาราง 17 ชนิดท้ายหน้า (SEO) · `/api/cat`:
  ล็อกอิน (guest 401) → ฟรี 1/บัญชี (bucket logic:22 — **id 22 ไม่ใช่ Logic ในสเปก 21 โมดูล** ใช้แค่
  bucket/quota/feedback) → 20cr `cat_reading` (costThb 0.35) → engine คำนวณใหม่ฝั่ง server (ไม่เชื่อ
  client) → ai2 (LALA_CAT_SYSTEM: กฎห้ามคำตัดสินร้าย/สุขภาพสัตว์ · Productive Clash เฉพาะเมื่อข้อมูลระบุ)
  → หักหลังสำเร็จ · เมนูหน้าแรก + sitemap 0.8 + FEEDBACK_MODES โหมดที่ 11 (migration 045 seed prompt
  รัน prod แล้ว) · CHAT_ENABLED_LOGICS/CHAT_LOGIC_NAMES/FREE_OVERRIDES เพิ่ม 22
- 🐛 **บั๊กที่ E2E จับได้:** ลืมใส่ 22 ใน CHAT_ENABLED_LOGICS → checkQuota ปฏิเสธตั้งแต่ครั้งแรก (429 ทั้งที่
  usage ว่าง) — โหมดใหม่ที่ใช้ bucket ต้องเพิ่มใน 3 ที่: ENABLED/NAMES/FREE_OVERRIDES
- **verify:** E2E จริง (test user + AI 5.2 วิ): โกญจา · น้ำ↔ไฟขาดน้ำ = Productive Clash +2 ตรง engine ·
  ชื่อส้มโอ→ดิน · ครั้ง 2 → 429 · bucket logic:22 used 1 · cleanup · anon 401 · coat ปลอม 400 ·
  browser: ฟอร์ม/ตาราง 18 รายการ/กล่องความเห็นขึ้น · eslint ทุกไฟล์ · เทสต์ 567

### ✅ เครดิตทดลองจากความเห็นรายโหมด (6 ก.ย. 2569 ผู้ใช้เคาะ — ช่วงทดลองที่ยังต่อ Omise ไม่ได้)

วงจร: ใช้โหมด → ให้ความเห็น → **รับ 20 เครดิต** → ลิงก์พาไปลองโหมดที่ยังไม่ได้ลอง → วนต่อ
- `migration 044` (รัน prod แล้ว): feedback_prompts เพิ่ม `logic_id` (unique — 1 prompt/โหมด =
  กันฟาร์ม "ครั้งเดียวต่อโหมด" ผ่านกลไก count เดิม) + `reward_credits` · seed คำถาม 10 โหมด
  (แอดมินแก้ทีหลังได้) · prompt เดิม logic_id null = รางวัลเดิม +1 คำถามฟรี ไม่เปลี่ยน
- `lib/feedback/mode-reward.ts` (3 เทสต์ → รวม 562): registry 10 โหมด (เพดาน 200cr/บัญชี ≈
  ต้นทุน AI ~฿4-8/คน) · `decideModeReward` pure — ล็อกอิน + **บัญชีถาวรเท่านั้น** (guest ชวนผูก) +
  ครั้งเดียว + ความเห็น ≥20 ตัวอักษร + โหมดที่มีหลักฐาน server (ฝัน/เสี่ยงทาย/เนื้อคู่ = bucket
  logic:X) ต้องใช้จริงก่อน — โหมด client ฿0 ไม่มีบันทึก ประกาศตรงๆ ว่าพึ่งชั้นอื่นแทน
- `/api/feedback`: GET ?logicId → prompt+สถานะเคลม+โหมดที่ยังไม่ลอง · POST เดิมขยาย —
  prompt รายโหมดให้เครดิตจริงผ่าน grant_credits (ledger `feedback:logic:X`) + คืน suggestModes
- `ModeFeedback.tsx` วางครบ 10 หน้าโหมด: anon = CTA ล็อกอิน/ผูกบัญชี "รับ 20 เครดิตฟรี" ·
  เคลมแล้ว = ชิปลิงก์โหมดที่เหลือ · ได้เครดิต = syncAuthStatus อัปเดตชิปทันที
- **verify**: prod DB จริง (seed 10 โหมด · grant+ledger · count กันซ้ำ · cleanup) · dev server:
  GET ตอบถูก (logicId ปลอม = prompt null) · browser /timing + /dream กล่องขึ้น ไม่ชนช่องแชท ·
  eslint ทุกไฟล์ที่แตะ (บทเรียน 2 ก.ย.) · เทสต์ 562
- ⏳ ตอนเปิดจ่ายจริง: พิจารณาปิด/ลดรางวัล (แอดมินปิด prompt รายโหมดได้เลย — คอลัมน์ active เดิม)

### ✅ แก้ช่องพิมพ์ /dream หายหลังทำนายครั้งแรก (3 ก.ย. 2569 — ผู้ทดลองรายงาน)

**ต้นเหตุ:** หน้า /dream ซ่อนฟอร์มทั้งแถบเมื่อ `remaining === 0` — ฝันฟรีมี 1 ครั้ง/บัญชี
พอใช้สิทธิ์แรกช่องพิมพ์หายถาวร ทั้งที่ route รองรับหักเครดิตต่อ (20cr) และ tester ไม่หักด้วยซ้ำ
(เงื่อนไขนี้เป็นซากตั้งแต่ยุคก่อนมีระบบเครดิต) **แก้:** ฟอร์มโชว์เสมอ + บรรทัดบอกเรทตรงๆ
"สิทธิ์ฟรีครบแล้ว — ครั้งต่อไปหัก 20 เครดิต" เมื่อ remaining=0 · เครดิตไม่พอ = 429 + ปุ่มเติม
ใน bubble (มีอยู่แล้ว) · ตามบทเรียน 2 ก.ย.: รัน npx eslint ไฟล์ .tsx ก่อน push แล้ว

### ✅ แท็บกลับหน้าแรกทุกโหมด (2 ก.ย. 2569 ผู้ใช้สั่ง)

`app/_components/HomeTab.tsx` ใน root layout จุดเดียว — พิลล์ "🏠 หน้าแรก" fixed มุมซ้ายบน
ทุกหน้า (ซ่อนเฉพาะ "/") · สไตล์ชุดเดียวกับชิปสถานะขวาบน (พื้นเข้ม #221c14 + ตัวทอง) ·
จอ <430px เหลือไอคอน 🏠 อย่างเดียว (กันชนชิปผู้ล็อกอินที่กว้างสุด 72vw) · verify browser จริง:
ตำแหน่ง/ไม่ overlap มือถือ/คลิกกลับ "/" จริง/หายบนหน้าแรก · ทดลอง A/B โมเดลภาพเนื้อคู่พ่วง
(schnell vs dev): **คง schnell** — dev สวยกว่าเล็กน้อยแต่ท่าคอลลาจซ้ำ 4 ช่อง + ลุค/ผิวเพี้ยนกว่า
+ แพง ~8 เท่า (บันทึกไว้กันเสนอสลับซ้ำ)

### ✅ แก้เชื้อชาติ/โทนผิวหลุดในคอลลาจเนื้อคู่ (2 ก.ย. 2569 — ผู้ทดลองเลือกไทย+ผิวสองสี ได้ภาพฝรั่ง)

**ต้นเหตุ:** บั๊กตระกูลเดียวกับวลีรูปลักษณ์ (1 ก.ย.) — การย้ายรูปลักษณ์+มิดชิดขึ้นต้น prompt ทำให้
วลีสัญชาติ (`${look}`) และโทนผิว (skinEn) ตกค้างท้าย prompt แพ้น้ำหนัก **แก้สูตรเดิม:** ฝังเข้า
ประโยคแรกติดตัวบุคคล ("adult woman with Thai appearance…, warm medium skin tone, fully
dressed…") + ย้ำระดับ panel ("the same ethnicity and the same skin tone in all four panels") ·
ตัดตำแหน่งท้ายเดิมออก (ตัวเก่า soulmateImagePrompt โดน replace รวม — คืนให้แล้ว มีเทสต์จับ)
**ยิง FLUX จริง 2 รูป (ไทย+ผิวสองสี+แบบ ข):** ลุคไทยชัด ผิวสองสี มิดชิด หน้าเรียวสูงเพรียวครบทุกช่อง
🔴 **บทเรียนสรุปรวม 3 เหตุการณ์ (รูปลักษณ์/เพศ/เชื้อชาติ):** ทุกคุณสมบัติที่ "ต้องชนะ" ใน prompt
FLUX ต้องอยู่ในประโยคแรกติดตัวบุคคล + ย้ำระดับ panel — การดันเรื่องใหม่ขึ้นต้นต้องตรวจว่าไม่ดัน
ของเดิมหลุดตำแหน่งเสมอ (ตอนนี้ประโยคแรกถือครบ: เพศ → สัญชาติ/ผิว → มิดชิด → รูปลักษณ์)

### ✅ แก้ภาพเนื้อคู่ออกมาดำสนิท (2 ก.ย. 2569 — ผู้ทดลองรายงานภาพดำทั้งใบ)

**ต้นเหตุ (พิสูจน์ด้วยการเจนซ้ำ prompt เดิมจาก image_generation_log):** การย้ำรูปลักษณ์รอบก่อน
ใส่คำสั่งเชิง "โชว์ร่างกาย" (clearly showing her body build / body silhouette) → FLUX ตีความเป็น
เสื้อเกาะอก/เปิดไหล่ → บางรอบโดน **safety checker ของ fal แทนภาพเป็นสีดำทั้งใบ**
**แก้:** ผูก "fully dressed in modest clothing with covered shoulders" เข้าประโยคแรกติดตัวบุคคล ·
ตัดวลี showing body build/silhouette (ความตรงแบบมาจากวลีรูปลักษณ์เอง ไม่ใช่คำสั่งโชว์) ·
outfit ท้าย prompt เป็น "modest fully-covering … with sleeves" · ยิง FLUX จริง 2 รูป: มิดชิด+
หน้ายาวสูงเพรียวครบทุกช่อง ไม่มีภาพดำ
🔴 **บทเรียน:** อยากให้ภาพตรงรูปร่าง ให้เข้มที่ "วลีบรรยายรูปลักษณ์" — ห้ามเพิ่มคำสั่ง "show
the body" เด็ดขาด (ชนตัวกรอง NSFW) · ภาพดำจาก fal = ธง safety ไม่ใช่บั๊กเรนเดอร์
⚠️ อีกบทเรียน: `falSoulmateImages` คืน `{url,...}` ตรงๆ ไม่ใช่ `{images:[...]}` — probe ที่อ่าน
`r.images[0].url` จะได้ undefined เงียบๆ (เคยทำ verify หลอกไปหนึ่งรอบ)

### ✅ คำบรรยายภาพตามเส้นทาง ก/ข ที่เลือก (1 ก.ย. 2569 — ผู้ทดลองทัก: เลือกแบบ ข แต่คำบรรยายเป็นทางตำรา)

**อาการ:** เลือกแบบ ข (ไม้ — หน้ายาว สูงโปร่ง) ภาพออกตามแบบ ข ถูกต้อง แต่คำบรรยายใต้ภาพ
(นิสัย/รูปลักษณ์/เคมี) ยังเป็นของ reading หลักทาง ก (น้ำ: หน้ากลม/ท้วม) — soulmateImageCaptions
ไม่รู้จัก pathChoice
**แก้:** `soulmatePathImageCaptions(path)` ใหม่ — ทุกช่องจาก SoulmatePath จริงของทางที่เลือก
(traitsTh ค.1 / appearance ค.1 / styleTh / chemistry / ทิศ) · route โหมด images: มี pathChoice
+ dualPath → ใช้ตัวนี้ (ทั้ง ก และ ข — เลือก ก ก็ได้ captions ของ ก ที่สอดคล้องภาพ) ·
⚠️ คงตัวคั่น " · แนวโน้มรูปลักษณ์ตามนรลักษณ์: " เป๊ะ — OG/สตอรี่ /sm split บรรทัดรูปลักษณ์จากตัวนี้
· เทสต์ล็อกห้ามปนรูปลักษณ์ทางตำราเมื่อเลือก ข (558)

### ✅ เลนส์เลขตัวตนเข้าทุกโหมดที่คำนวณผู้ใช้ (1 ก.ย. 2569 ผู้ใช้เคาะ "ทั้งสองกลไก + สองทางให้เลือก")

ผู้ใช้สั่ง: โหมดที่นำผู้ใช้ไปคำนวณ ควรนำ "ค่าพลังงานจากการ์ดพลังงาน" เข้าไปด้วย — เคาะผ่าน
AskUserQuestion: ใช้ทั้งคะแนน 5 ด้าน + ธาตุจากเลข โดยเล่าเป็นสองทางให้ผู้ใช้เลือก · ทุกโหมดที่เลือก
- `lib/engine/identity-lens.ts` (3 เทสต์ → รวม 557): identityLens (เลข/การ์ด/ธาตุจากเลข/aspects —
  ประกอบจาก personalEnergyNumber + lookup2digit + artifactElement + partAspects เดิมล้วน) ·
  identityDualChemistry (ทาง ก มี missing/clash · ทาง ข ไม่มี — เทสต์ล็อก) · summaryTh จุดเดียว
  🔴 **เลขตัวตนต้องมาจากข้อมูลโปรไฟล์ชุดเดียวกับการ์ด /profile เสมอ** (บทเรียนเดียวกับการ์ดชั้นชื่อ)
- **เสียบครบตามที่เคาะ:** แชท (fn ที่ 22 `myIdentityAspects` — ctx.identity server เติมจากโปรไฟล์
  ชื่อ+เวลา+วันเกิด AI ไม่เห็นข้อมูลดิบ · allowlist 21→22) · first-reading (บล็อก+กฎ ①.5 สองมุม
  ห้ามเฉลี่ย) · oracle (โปรไฟล์ RLS own-row → context + กฎท้าย system · พังไม่ล้มพิธี) ·
  timing/fengshui/wellness (`IdentityLensNote` component เดียว — บล็อก 🔢 กาง/พับ + บรรทัดเฉพาะโหมด:
  วันบำรุงเลนส์/ทิศบำรุงเลนส์/เทคนิคธาตุเลนส์ — **ไม่เข้าสูตรคะแนนเดิมของโหมดใดๆ**)
- caveat ผ่านกติกาภาษาผู้ใช้ (เทสต์เดิมจับคำ "ตาราง" — แก้เป็น "หลักเลขศาสตร์แปลงเลขเป็นธาตุ")
- **ยิง AI จริง:** "เลขตัวตนของฉันเป็นยังไง" → เลข 59 ตรงมือ (28+5+3+23) · ธาตุดินจากเลขถูก ·
  คะแนน 5 ด้านตรง engine ทุกตัว · เล่าคู่ธาตุกำเนิดแบบสองมุมไม่เฉลี่ย

### ✅ การ์ดชั้นชื่อเนื้อคู่ = การ์ดพลังงานสูตรรวม (1 ก.ย. 2569 — ผู้ทดลองทักการ์ดสองจุดไม่ตรงกัน)

**อาการ:** แถว "เลขศาสตร์ · การ์ดพลังชื่อ" ในเนื้อคู่เปิดการ์ดจากเลขชื่อล้วน (reduceTo99(namePower))
ไม่ตรงการ์ดพลังงานหน้า /profile — ขัดมติสูตรรวม 31 ส.ค.
**แก้:** nameNumerology/buildOwnNameLayer/readings รับ `energy {birthDate, birthTime}` →
การ์ด = personalEnergyNumber (Birth+Name+Time+Day) **ตรงการ์ด /profile เป๊ะ** · เลขศาสตร์ชื่อ
(namePower) คงเดิม — เป็นเลขของชื่อ ไม่ใช่การ์ด · โหมด match การ์ดฝั่งเขา = พลังงานส่วนบุคคล
ของเขา (วันเกิด+ชื่อ+เวลาเขา เท่าที่มี) · ไม่ส่ง energy = พฤติกรรมเดิม (เทสต์ล็อกทั้งสองทาง 554)
· ป้าย UI → "เลขศาสตร์ชื่อ · การ์ดพลังงาน" + context key สื่อความจริง
...
โครงคำทำนายส่วนอื่นคงเดิม · กลไกทางแยก ก/ข ของเนื้อคู่ (เมื่อสองศาสตร์ขัด) เป็นคนละกลไก คงเดิม
- `lib/engine/dual-advice.ts` (4 เทสต์ → รวม 552): สองแนวคือเส้น +2 ของ wuXingScore "ทาง ค"
  ที่มีอยู่แล้ว — เสริมขาด = ธาตุขาด (Productive Clash · TTM หนุน) / เสริมแข็ง = ธาตุแม่ 印
  (ประกอบจากวงจรให้กำเนิด **ไม่ใช่ตารางตำราเฉพาะ** — sourceTh สองแนวต่างกันจริง เทสต์ล็อก) ·
  ประกอบจาก ELEMENT_TO_COLORS + WELLNESS_ACTIVITIES ล้วน ไม่มีสูตรใหม่ · ธาตุครบ = lack null
  ไม่มีทางแยกปลอม · SUPPORT_OF ย้ายจาก plan.ts มาเป็นแหล่งเดียวที่นี่
- **กติกา Mirror เดิมของเนื้อคู่ยกมาทั้งชุด**: treatment เท่ากัน · ห้ามคำเชียร์ (เทสต์ regex ล็อก
  ทุก string) · ระบบไม่เลือกแทน · "สองแนวไม่ขัดกัน ใช้ร่วมกันได้" (ต่างจากภาพเนื้อคู่ที่เลือกใบเดียว)
  · เหตุผลห้ามเชียร์เชิงศาสตร์: ระบบไม่คำนวณกำลังวันเกิด 身強/身弱 (เหตุเดียวกับ "ทาง ค" ไม่ให้ −1)
- **เสียบ 4 ผิว (บล็อก <แนวคำแนะนำสองแบบ> + กฎ 1 ข้อ/จุด)**: แชทปรึกษา (plan-run — ก้าวเล็ก
  สองแนวเมื่อมีโปรไฟล์) · first-reading (④.5) · องค์รวม narrate (④ — client แนบบล็อกจาก seed) ·
  เนื้อคู่ reading (⑤ ทั้งเคส Mirror และพอร์เทรตเดียว) — dream/oracle/match ไม่แตะตามขอบเขต
- **ยิง AI จริงพิสูจน์**: คำถามเครียดงาน (ไฟเด่นขาดน้ำ) → ก้าวเล็กสองแนว สี/เทคนิคตรง engine
  (น้ำเงิน-ฟ้า+Bhramari / เขียว+Kapalabhati ไม้บำรุงไฟ) · ไม่มีคำเชียร์ · ปิด "การตัดสินใจเป็นของคุณ"

### ✅ พลังงานบุคคลสูตรรวมทุกโหมด (31 ส.ค. 2569 ผู้ใช้เคาะ — ผู้ทดลองยืนยันสูตรรวมตรงกว่า)

ยกเครื่องนิยาม "เลขตัวตน/พลังงานส่วนบุคคล 00-99" ทั้งระบบตามมติใหม่ (ดู §4 ข้อ 1 ฉบับแก้):
- `computeCardId`: **TimePower ลดทอนเหลือหลักเดียว** (จุดเปลี่ยนเดียวของสูตรการ์ด — องค์ประกอบอื่น
  ตรงข้อเสนออยู่แล้ว) · regenerate cardid.fixture (generator อิสระแก้คู่กัน — เปลี่ยน 2 เคส)
- `personalEnergyNumber(birthDate, {name?, birthTime?})` ใหม่ใน card-id.ts — สูตรเดียว ส่วนที่ไม่มี=0 ·
  **แทน `birthPowerNumber` ทุกจุด** (network-holistic re-export · องค์รวม self+persons ได้ Birth+Day ·
  partnerMatchReading รับ userBirthTime/userName/partnerBirthTime เพิ่ม — route ส่งให้เมื่อมี)
- ไม่มีข้อยกเว้นเลขตอง (ผู้ใช้เลือก "ลดทอนปกติ") · ตาราง NamePower ไม่แตะ (ตรง verify เดิม)
- ผลข้างเคียงที่รับทราบ: การ์ด/เลขตัวตนผู้ใช้เดิมบางส่วนเปลี่ยน (เวลาผลรวม ≥10 + เลขตัวตนบวก Day) ·
  เทสต์ล็อกใหม่: 1986-10-07→35 · 1990-03-15→33 · +3 เมื่อ 18:30 · +23 เมื่อชื่อสมชาย — 548 ผ่าน
...
แบบน้ำ — วลีรูปลักษณ์ ค.1 อ่อนเกิน+จมกลาง prompt ยาว **แก้ 3 จุด:**
① วลีรูปลักษณ์ย้ายขึ้นต้น prompt + emphasis "clearly and unmistakably has: ... — must be
visible in every panel" ② pathChoice ใช้ `ELEMENT_APPEARANCE_STRONG_EN` (รวม body+face promptEn
ของธาตุทางนั้นจากตาราง preset — เดิมส่ง [] เหลือแต่วลี ค.1 อ่อน) ③ log brand_element บันทึกธาตุ
ของภาพจริง (เดิมบันทึกธาตุตำราเสมอ ทำให้ debug หลงทาง) · **ยิง FLUX เทียบ ก/ข แล้วต่างชัด:**
ก=หน้ากลมนุ่ม พื้นฟ้าอ่อน · ข=หน้ายาวเพรียวสูง

### ✅ คำทำนายสองแนวทางแบบ "โครงกระจก" (25 ส.ค. 2569 ผู้ใช้เคาะหลังขอคำแนะนำ)

ยกเครื่อง LALA_SOULMATE_SYSTEM รอบสอง: **ตัดหัวข้อ "หลักการคำนวณ" ออก** (ที่มาแสดงบนหน้าจอ
อยู่แล้ว — ความโปร่งใสไม่หาย) + กรณีมีทางแยก ใช้ **Mirror Structure**: แนวทาง ก/ข หัวข้อย่อย
ชุดเดียวกันทุกหัวข้อ (นิสัย/รูปลักษณ์/เคมี+ความยั่งยืน/ชีวิตคู่+สิ่งที่ดูแล) · ความยาวต่างกัน
ไม่เกิน ~2 ประโยค · **ห้ามคำเชียร์** (ดีกว่า/เหมาะกว่า/แนะนำทาง/ควรเลือก) · **ห้ามป้ายลำดับชั้น
ในหัวข้อ** (ทางหลัก/ทางรอง — ป้ายที่มาอยู่บนการ์ด UI ตามจริง แต่การเล่า treatment เท่ากัน) →
จุดร่วม-จุดต่าง → คำแนะนำที่ใช้ได้ทั้งสองทาง + caveat · ไม่มีทางแยก = พอร์เทรตเดียวเต็มเหมือนเดิม
🐛 **บั๊กที่ E2E จับได้:** กฎข้อ 2 เดิมสั่ง "อธิบายเป็น 'หลักการคำนวณ' แทน" ขัดกับกฎใหม่ห้ามใช้วลีนี้
— Flash เลยหลุดทุกรอบ แก้ rule ให้สอดคล้อง (อ้างที่มาแบบสั้นในเนื้อความแทน) · E2E สุดท้าย:
สมดุล ก:ข = 1.09 · ไม่มีวลีต้องห้าม/ป้าย/คำเชียร์/ชื่อศาสตร์ · บรรทัดปิดครบ

### ✅ ทางแยกอัตโนมัติ + คำทำนายไร้ชื่อศาสตร์ (25 ส.ค. 2569 ผู้ใช้ทักจาก screenshot จริง)

① **Dual Path ขึ้นอัตโนมัติ** — เดิมต้องกรอกสเปกก่อน (ผู้ใช้ทัก "ยังไม่มีให้เลือก"): ตอนนี้ไม่กรอก
   ก็ชี้ทางแยกเมื่อมีธาตุอื่นเคมี ≥ ทางตำรา (จาก rankedElements ของ engine — ไม่ใช่การเดา) ·
   sourceTh ทาง ข แบบ auto: "อีกเส้นทางบวกที่ดวงคุณรองรับ" · กรอกสเปก = override เป็นธาตุจากสเปก
② **ห้ามเอ่ยชื่อศาสตร์ในคำทำนาย** (Jyotish/BPHS/Vimshottari/Ashtakoota/ค.1/ชื่อตำรา) —
   ยกเครื่อง LALA_SOULMATE_SYSTEM โครงใหม่ตามที่ผู้ใช้สั่ง: ① หลักการคำนวณที่ใช้ (ภาษาชาวบ้าน)
   → ② ผลการคำนวณ+ทำนายรายด้าน → ③ คำทำนายแยกตามทางเลือก ก/ข → ④ คำแนะนำ+caveat ·
   **rename context keys** (ชั้นJyotishสากล→ชั้นคำนวณตำแหน่งดาว ฯลฯ — กัน AI สะท้อนชื่อจาก key) ·
   caveat/หัวข้อบนหน้าถอดชื่อศาสตร์ทั้งหมด (JYOTISH_CAVEAT/TIMING/SCOPE_NOTE/convergence detail
   เปลี่ยนเป็น "ชั้นคำนวณตำแหน่งดาว" — เนื้อหาความซื่อตรงคงเดิม) · บรรทัดปิด "การเลือกเป็นของคุณ"
   แนบ deterministic (บั๊กเดิม: replace เงียบไม่เข้าเพราะไม่มี assert — เข้าใหม่ด้วย Edit แล้ว)
   **E2E:** ไม่กรอกสเปก → ก=น้ำ(+2)/ข=ไม้(+2) อัตโนมัติ · reply ไม่มีชื่อศาสตร์สักคำ · โครง ①-④ ครบ
...
① **ตาราจรเฉพาะบุคคลใน /timing**: PersonLayer เพิ่ม birthNak (จันทร์ ณ เที่ยงวันเกิด — convention
   เดียว ashtakoota no-time) → ทุกวันคำนวณตารา 9 (นับนักษัตรเกิด→นักษัตรวัน · เศษ 3/5/7 = วิปัต/
   ปรัตยริ/วธะ ไม่เกื้อ −1 · 2/4/6/8/9 เกื้อ +1 · ชนมะกลาง) — สูตรเดียวกูฏตาราที่วิจัย/ล็อกแล้ว ·
   โน้ต 🌙 รายวัน + caveat ประกาศ · เทสต์: 27 วันต้องมีทั้งบวก/ลบ + คะแนนเปลี่ยนจริง
② **ทศาสันธิเข้า /timing**: ช่องเวลาเกิด (ไม่บังคับ · prefill จากบัญชี) → อยู่รอยต่อยุค = โน้ต 🔶
   ใน caveat "เหมาะสังเกตมากกว่าผูกมัดเรื่องใหญ่" (ชั้นเสริม ไม่กระทบคะแนน · พังไม่ล้มผล) —
   คำเตือนจากวิจัยรอบ 2 มาถึงคนเลือกฤกษ์เรื่องใหญ่ซึ่งควรเห็นที่สุด · เทสต์ล็อกทั้งมี/ไม่มีเวลา
③ **ทักษาแบรนด์ใน /logo**: `brandNameTaksa()` (logo-team.ts — analyzeNameTaksa × วันเกิดเจ้าของ ·
   กัน พ.ศ. **ช่องที่เทสต์จับได้เอง** — เดิมไม่ guard ปี) · แถว ✍️ เตือนอักษรกาลกิณี + อักษรแนะนำ
   เดช/ศรี · browser จริง: "กาแฟดี"×เจ้าของวันอังคาร → เตือน ก + แนะ ดตถทธน/บปผฝพฟภม ✓
   เทสต์รวม 545
...
convergence) · สูตรสำเร็จ: ค้นกฎ cross-check ≥2 แหล่ง → เอนจิน+เทสต์ → ทดลองสุ่มหลายเคส → จูน → E2E

1. ✅ **รอบ 1 /timing: ฤกษ์บน 27 ฤกษ์ — เสร็จ 24 ส.ค. 2569 (ปิดหนี้ §3.6 ฤกษ์บน)**
   `lib/engine/rerk.ts` (5 เทสต์ → รวม 521): นักษัตร 27 ชื่อไทย (วิกิพีเดีย) + นพเคราะห์ฤกษ์ 9 กลุ่ม
   **วงจร k, k+9, k+18** ลำดับ ทลิทโท→มหัทธโน→โจโร→ภูมิปาโล→เทศาตรี→เทวี→เพชฌฆาต→ราชา→สมโณ
   (ยืนยัน 3 แหล่งอิสระ: วิกิพีเดียไทย/Sanook/aduang + TrueID/fengshuix/อาศรมศรีจักรนารท) ·
   ความหมาย+ความเหมาะราย activity 5 หมวด (⚠️ จับคู่หมวดสมัยใหม่=ตีความ มี RERK_CAVEAT) ·
   `moonRerkForDay` จันทร์นิรายนะ ณ เที่ยงไทย (แพทเทิร์นกาลกิณี) — **verify 60/60 วันกับ Swiss
   Ephemeris** · เสียบ rankAuspiciousDays เป็นชั้นที่ 3: คะแนน ±2 (กาลโยค ±3 > ฤกษ์ ±2 > ธาตุ ±1)
   + DayRanking.rerk ทุกวัน + activityKey param + แถว 🌙 บนหน้า /timing
   🔴 **จูนจากการทดลอง 45 วัน × 3 หมวด:** เดิมขึ้นบ้านโดน "ควรเลี่ยง" 47% → `hardAvoid` เฉพาะ
   ฉินทฤกษ์ (โจโร/เพชฌฆาต — ทุกแหล่งตรงกัน "ห้ามงานมงคล") ผลัก verdict ได้ · เทศาตรี (สำนัก
   ไม่ตรงกัน) หักคะแนนแต่ไม่ผลัก verdict → เลี่ยงเหลือ ~36% ตรงตำราสายกลาง · วันแย่สุดซ้อนชั้น
   ถูกต้อง (เพชฌฆาต+อุบาทว์ = −5)
   🔴 **จงใจไม่แสดงดาวเจ้าฤกษ์** — สำนักไทย (เรียงทักษา: ทลิทโท=อาทิตย์) ขัดสำนักแขก (Vimshottari:
   ทลิทโท=เกตุ — โน้ตเก่า §3.6 มาจากระบบนี้) ฟังก์ชันไม่ต้องใช้จึงตัดทิ้ง ไม่เลือกข้าง
2. ✅ **รอบ 2 /fortune + first-reading: ยุคชีวิต Vimshottari — เสร็จ 24 ส.ค. 2569**
   `lib/engine/life-dasha.ts` (4 เทสต์ → รวม 525): MD_THEME_TH 9 ดาว (Phaladeepika บท 19 +
   JagannathHora + BPHS — cross-check) · Naisargika Maitri **ทิศทางเดียว** (มุมมองเจ้ามหาทศาต่อ
   เจ้าอันตร — ตารางไม่สมมาตร เทสต์ล็อก จันทร์→พุธ=มิตร แต่พุธ→จันทร์=ศัตรู · แถวราหู/เกตุ=
   ธรรมเนียมยุคหลัง) · dasha-sandhi = อันตรสุดท้าย/แรก (สำนักต่างกัน — caveat) · `lifeDasha()`
   บนเอนจิน Vimshottari ที่ verify แล้ว — อายุเกิน 120 ปี → null
   🔴 **กรอบห้าม maraka เชิงโครงสร้าง:** เทสต์ regex ล็อกทุก string ห้ามคำ ตาย/มรณ/อายุขัย/โรคร้าย
   (เจอจริง: "ตายตัว" ชนตัวกรอง → เปลี่ยนถ้อยคำ) · เสาร์/ราหู/เกตุ/อังคาร เฟรม "งานหนัก/วินัย/
   เปลี่ยนแปลง"+ด้านโอกาสเสมอ ("แรงเสียดทานไม่ใช่คำตัดสิน" — จากแหล่งวิชาการเอง)
   **wiring:** /fortune ส่วน 🪐 ยุคชีวิต (client ฿0 — จันทร์ ณ เวลาเกิดจริง) · first-reading ดึง
   birth_time เพิ่ม → ยุคชีวิตเข้า prompt เป็นชั้นเสริม (ห้ามขัด/แทนปีส่วนบุคคล · try/catch พังไม่ล้ม)
   **ทดลอง 5 ดวง:** ยุค/อายุ/รอยต่อ/มิตร-ศัตรูหลากหลายถูกต้อง (ดวงกันย์ 2529 = เกตุ/จันทร์ ตรง E2E
   เนื้อคู่) · **E2E จริง:** first-reading เล่า "ยุคเกตุ...ช่วงย่อยจันทร์...แรงเสียดทาน...ด่านทดสอบ"
   เฟรมถูกครบ · browser /fortune แสดงส่วนยุคชีวิตจริง
   ⚠️ เกร็ด E2E: user_profiles_e ไม่มีคอลัมน์ display_name (ใช้ first_name) — insert โปรไฟล์ใน
   E2E เก่าบางตัวพังเงียบโดยไม่กระทบผลเพราะ route ใช้ข้อมูลจาก body
3. ✅ **รอบ 3 Ashtakoota คะแนนคู่ 36 — เสร็จ 24 ส.ค. 2569**
   `lib/engine/ashtakoota.ts` (9 เทสต์ → รวม 534): ตารางครบจากงานวิจัย (Drik Panchang/Saravali/
   mPanchang ฯลฯ) — วรรณะ1 วัศยะ2 ตารา3 โยนิ4 คระหะไมตรี5 คณะ6 ภกูฏ7 นาฑี8 = 36 ·
   โยนิ 14×14 consensus (2 แหล่งต่าง 3 ช่อง ซ่อมด้วยสมมาตร — คู่ศัตรู 7 คู่ทุกแหล่งตรง) ·
   คระหะไมตรี 7×7 ตรวจตรงสูตร BPHS · ธนู/มังกรแบ่งครึ่งราศีในวัศยะ · แปลผล 4 เกณฑ์ + ธงโทษแยก
   (นาฑีโทษ/ภกูฏโทษ) · **เทสต์ยึดผลอ้างอิงสาธารณะ: นักษัตรเดียวกัน = 28/36**
   🔴 ทางที่ล็อกในจุดสำนักไม่ตรงกัน (ประกาศใน header+caveat): ตารา 3/5/7 · คณะไม่ผูกทิศทาง ·
   วัศยะ matrix Saravali · maitri สเกล 1/0.5 · **วรรณะ/วัศยะมีทิศทาง**: รู้บทบาท (เพศสองฝ่าย) →
   คลาสสิก · ไม่รู้ → เฉลี่ยสองทิศทาง (ประกาศ) · ไม่ทำกฎยกเว้นนาฑี/Mangal dosha (บอกใน caveat)
   🔴 กรอบเล่า: ห้ามชื่อชั้นวรรณะเชิงสังคม (ใช้ "กลุ่มธาตุจิตใจ") · นาฑี="พลังชีวิต" ห้ามคำแพทย์/บุตร
   (เทสต์ regex ล็อก) · ห้ามเฉลี่ยรวมกับคะแนน 5 ด้าน/เคมีธาตุ (กติกา §4)
   **wiring:** โหมด match เนื้อคู่ (มีเวลาเกิดทั้งคู่ · บทบาทจากเพศสองฝ่าย) — response.matchKoota +
   context + กฎ narrator 4.4 + บล็อก 🌙 บนหน้า · /compatibility ประเภทบุคคล — client ล้วน ณ เที่ยงวัน
   (วันเกิดบุคคลไม่ขึ้น server เหมือนเดิม · narrate ส่งเฉพาะคะแนนสรุป) + koota ใน HolisticPart +
   แอคคอร์เดียน + กฎ narrate 3.5
   **ทดลอง 5 คู่:** 18.5-31.5/36 ธงโทษ/เศษทิศทางถูก · **E2E จริง:** match 24.5/36 วิศาขา×จิตรา —
   AI เล่า "โยนิเสือ×เสือ 4/4" ตรง mapping จริง · browser /compatibility: บล็อกกูฏ+ดูรายกูฏ 8 ขึ้นจริง
   · เดโม caveat no-time ของจริง: ดวงเดียวกันวันขอบนักษัตร เที่ยงวัน=อนุราธา vs เวลาจริง=วิศาขา
   (ระบบเตือนตรงตามออกแบบ)
4. ✅ **รอบ 4 ทักษาปกรณ์ตั้งชื่อ — เสร็จ 24 ส.ค. 2569 (แผนแม่บท 4 โหมดครบทุกรอบ 🎉)**
   `lib/engine/taksa-naming.ts` (4 เทสต์ → รวม 539): อักษร 8 วรรคตามดาวเจ้าของวรรค (4 แหล่งไทย
   ตรงกัน: บุญบารมี/TheLuckyName/Sanook/MThai) · **generate ผัง 8×8 จากกฎหมุนมหาทักษา**
   (บริวาร=วรรคดาววันเกิด → ไล่ตามเข็ม อาทิตย์→จันทร์→อังคาร→พุธ→เสาร์→พฤหัส→ราหู→ศุกร์) —
   **validate ไขว้: ช่องกาลกิณีตรงลิสต์อิสระของ Sanook ครบ 8/8** (เทสต์ล็อก) · ความหมาย 8 ภูมิ ·
   กฎเหล็ก: อักษรวรรคกาลกิณีห้ามใช้ · ธรรมเนียมชายเน้นเดชนำ/หญิงเน้นศรีนำ (= ความนิยม ไม่ใช่ข้อห้าม
   · ไม่ระบุเพศ = ไม่แนะนำ ไม่เดา)
   🔴 ธรรมเนียมที่เลือก (สำนักไม่ตรงกัน — ประกาศใน caveat): ั/์/วรรณยุกต์ไม่นับทักษา · ฤ นับกลุ่มสระ
   (อาทิตย์) · พุธกลางคืนเริ่ม 18:00 · ก่อนรุ่งเช้าพฤหัสนับเป็นพฤหัสตามวันปฏิทิน (สอดคล้องชั้นอื่น
   ของระบบ) · ⚠️ "อ" อยู่กลุ่มสระในทักษา แต่กลุ่ม 6 ในเลขศาสตร์ — **คนละระบบ ห้าม cross-wire**
   **wiring:** myNameMatch (แชท) — PlanProfileContext เพิ่ม dayOfWeekTh (server เติม AI ไม่เห็น) →
   ชั้น "ทักษาปกรณ์_ชั้นอักษรวรรค" ในผล (verdict/อักษรกาลกิณี/ภูมิที่พบ/อักษรแนะนำ เดช-ศรี-มนตรี) ·
   ctx เก่าไม่มีวัน = ไม่มีชั้นนี้ ไม่พัง (เทสต์ล็อก)
   **E2E จริง:** "ชื่อ กมลชนก เข้ากับดวงฉันไหม" (โปรไฟล์เกิดอังคาร) → AI เล่าชั้นเลขศาสตร์กลมกลืน
   แต่เตือน "ก" ×2 ตกวรรคกาลกิณีของวันอังคาร นุ่มนวล+caveat ครบ — สองระบบเล่าแยกชั้นถูกกติกา
...
1. ~~**โหมดความรักและเนื้อคู่**~~ — ✅ **v1 เสร็จ 21 ส.ค. 2569** (ดูบล็อก ✅ ด้านบน) ·
   งานเดิมที่วางไว้:
   - โครง: ตรวจเพศ (โปรไฟล์มี gender แล้ว + ถาม "เพศคู่ที่สนใจ" ห้ามเดา) → คำทำนายเนื้อคู่โครง 8 หัวข้อ
     ตามตัวอย่าง Jyotish ที่ผู้ใช้ให้ (นิสัยสองชั้น→ภายนอก→พื้นเพ→ฐานะ→อายุ→เจอที่ไหน→รูปแบบความสัมพันธ์→ช่วงเวลา)
     → เจนภาพเนื้อคู่ 1-3 รูป (คนสมมติ — ไม่มี PDPA ชีวมิติ, FLUX ~฿0.2/รูป)
   - แหล่งข้อมูลตาม §16: ลัคนานิรายนะ (verify แล้ว) → ราศีที่ 7 = ภพคู่ครอง → **ตารางลักษณะราศี**
     ที่ต้องเปิดตรวจก่อน: `data/raw-uploads/Zodiac_Signs_csv.csv` (ไม่เคยเปิด) + ตาราง ข.2
     "คุณลักษณะ 12 ลัคนาราศี" ในตำราจตุพลวัตร (ไม่เคยใช้) — ถ้าไม่พอให้ขอจากเจ้าของตำรา ห้ามแต่งเอง
...
3. **เอกสารทางเลือกตาราง 9 กลุ่มอักษร→ธาตุ** ส่งเจ้าของตำรารอบสอง (ท่านขอดูตัวเลือก)
4. **รอผู้ใช้ทำ (ทวงได้):** PAT เพิ่ม scope workflow → ผม push warm-og.yml (ไฟล์รออยู่
   scripts/github-workflows-pending/) · ตรวจ CRON_SECRET ใน Vercel + ตั้ง secret ใน GitHub ·
   แอป Meta สลับ Live (ขั้นตอนให้แล้ว — เหลือแก้ ToS URL + App domains + กดเผยแพร่) · GSC ดูเลข
   16→445 · ส่งไฟล์ Downloads/คำถามถึงเจ้าของตำรา_รอบสอง_ส.ค.2569.docx
5. ⚠️ ทุกครั้งหลัง deploy ที่จะมีการแชร์ทันที: อุ่นรูป OG 101 ใบ (สคริปต์ curl วน — ดูบล็อก warm-og)

### สภาพแวดล้อม (ต้องทำก่อนรันคำสั่งใดๆ)
```bash
export PATH="$HOME/.local/node/bin:$PATH"   # Node v24 ไม่อยู่ใน PATH ถาวร
cd /Users/freeman/Desktop/kruth-element
npx tsc --noEmit && npm test && npm run build   # ควรได้ 597/597 (25 ก.ย. 2569)
```
...
**✅ เฟส 2 อาณาจักรทำนายฝัน — เสร็จ 4 สไลซ์ (7 ส.ค. 2569):**
1. **เปิดใช้ `Unified_Kaekled_DB` ครั้งแรก** (ไฟล์อยู่ใน data/raw-uploads มาตั้งแต่ต้นโปรเจกต์ ไม่เคยเปิด)
   → `data/kaekled_remedies.json` + `lib/engine/kaekled.ts` (5 เทสต์): 8 สัญลักษณ์ที่คนกังวลสุด
   (ผี ศพ งู จระเข้ ฟันหัก เลือด ไฟไหม้ ถูกไล่ล่า) พร้อมแก้เคล็ด 3 ศาสตร์ + ธาตุเชื่อม + บทสวด
   🔴 **กฎที่ค้นพบและตรวจตรง 8/8: ธาตุเชื่อม = ธาตุที่สัญลักษณ์ให้กำเนิด** (น้ำ→ไม้ ดิน→ทอง ไฟ→ดิน
   ทอง→น้ำ) หลักคือ "ผ่อนแรงให้ไหลไปเป็นการเติบโต ไม่ปะทะกลับ" → ใช้กฎเดียวกันต่อยอดสัญลักษณ์อื่นได้
   แต่ประกาศที่มาต่างกันเสมอ (`source: "ตำราแก้เคล็ด" | "หลักธาตุ"` — เทสต์ล็อกว่าตัวนอกตำราห้ามมี
   แนวทางไทย/จีน/ฮินดู) · KAEKLED_CAVEAT บังคับ ("ไม่ใช่คำรับประกันผล")
2. **แก้ช่องว่าง matching คำประสม** — "งูเห่า" ICU มองเป็นคำเดียว จึงไม่เคยจับ "งู" ได้เลย:
   เพิ่ม compound-head match **เฉพาะหมวดสัตว์/แมลง** (หัวคำ ≥2 รูปอักขระ + ส่วนที่เหลือ ≥2)
   🔴 **ห้ามเปิดทั้งฐาน** — "ตาข่าย"→ตา "หัวใจ"→หัว จะพังทันที (มีเทสต์ล็อก "งาน" ห้ามเป็น "งา")
   ⚠️ บทเรียน: ห้ามใช้ตัวนับที่ตัดสระบน-ล่าง — "งู" จะเหลือ 1 แล้วโดนกฎตัดทิ้ง (ใช้ `glyphLen`)
   · เพิ่ม TRAP_PHRASES (แผ่**แม่**เบี้ย/**ต่อ**ไป/**ตา**ข่าย ฯลฯ) + dedupe + จัดลำดับคำนามรูปธรรม
   มาก่อนกริยา + จำกัด 8 รายการ (ฐานมีแถวซ้ำ 49 แถว "ผึ้ง/ต่อ/แตน" เคยโผล่ 3 ครั้งในคำตอบเดียว)
   · **เพิ่ม 2 สัญลักษณ์ที่ฐาน 457 ไม่มีเลยแต่ตำราแก้เคล็ดมี: "งู" และ "ไฟไหม้"** (อักษรจีนเติมจาก
   จำนวนขีดที่ตรงพอดี 蛇=11 火=4 = ยืนยันข้ามกัน ไม่ใช่เดา) — ใช้เฉพาะเส้น production ไม่แตะ golden
3. **คำตอบ /dream เป็นโครง 5 ส่วน "เล่าเป็นฉาก"** ตามตัวอย่างที่ผู้ใช้ให้: 🗝️ ความหมายหลัก →
   ⏳ ปัจจัยแวดล้อม (วัน+จังหวะฉาก+ความจำ) → 🔢 รหัสพลังงาน → 🕯️ แนวทางคลี่คลาย → 🔮 บทสรุป+ชวนคุยต่อ
   · **ตัด CTA "ขอเลขชุด" ของตัวอย่างเดิมทิ้ง** (ชน guardrail หวย) · UI ชวนเล่าเป็นฉากทั้ง bubble
   แรกและ placeholder · 🔴 **กฎใหม่ข้อ 1.1: สิ่งที่ผู้ใช้เล่าแต่ไม่อยู่ในรายการสัญลักษณ์ ห้ามให้ธาตุ/
   ขีด/ความหมายเชิงสัญลักษณ์** — จับได้จากการทดสอบจริง (AI แต่งความหมายให้ "แมว" ที่ไม่มีในฐาน)
4. **หน้า SEO ทำนายฝัน 398 หน้า + hub** (`/dream-meaning`, `/dream-meaning/<ชื่อไทย>`) — static ล้วน
   ฿0 ไม่เรียก AI/DB · เนื้อหาต่อหน้า: ความหมายจากฐาน + ตารางฝันวันไหนต่างกัน (คำนวณจากวงจรกำเนิด/
   พิฆาตจริง) + ขีดคังซี + แนวทางคลี่คลาย + FAQPage JSON-LD + ลิงก์ภายในสัญลักษณ์ใกล้เคียง ·
   sitemap 415 URL · เป้า "ทำนายฝัน" 274k/เดือน KD 23 · ⚠️ `dayRelationText` เขียนคำใหม่แทนการยืม
...
- **`.bg-triskele`** (globals.css) = พื้นน้ำเงินเข้ม #0a0e28 + ลาย `public/pattern-triskele.jpg`
  ...
  · ใช้กับ**หน้าที่เนื้อหาอยู่ในการ์ด gold-frame เท่านั้น**: `/` `/consent` `/welcome` `/login`
  🔴 **ห้ามใช้กับหน้าอ่านยาว** (SEO 428 หน้า/ผลดวง/บทความ — ยังหินอ่อนตามมติ §2 เดิม ตรวจแล้วไม่กระทบ)
- **ตัวหนังสือนอกการ์ด** บนพื้นลายต้องใช้ `--outer-ink/--outer-ink-dim/--outer-gold` (ครีม/ทองสว่าง)
  — ห้ามใช้ --ink ของ marble (จมหาย) · welcome/login เดิมเนื้อหาลอยบนพื้น → **ห่อ gold-frame แล้ว**
- **โทนมืดพิธีกรรม (.tone-night) เปลี่ยนสี**: ม่วงเข้ม #0a0714/#14101c → **น้ำเงินเมทัลลิก
  #080d24/#101733** ให้เป็นชุดเดียวกับลาย (dream/oracle ตรวจ browser แล้ว — อ่านชัดทุกส่วน)
- 🐛 กับดักที่เจอ: login มี `background: var(--bg)` แบบ inline ทับคลาสพื้น — inline ชนะ class เสมอ
  ต้องเอา background ออกจาก inline style ของหน้า ให้คลาสเป็นคนทาพื้น

**✅ บัญชีเครดิตไม่จำกัดของแอดมิน (10 ส.ค. 2569 — ผู้ใช้สั่ง):** whootthira@gmail.com
(auth_uid 98f9445c-…) ไม่จำกัดเครดิต — ทำที่ **ชั้นกระเป๋าจุดเดียว** (lib/credits/wallet.ts):
`hasUnlimitedCredits()` → getCreditBalance คืน 999999 เสมอ + spendCredits เป็น no-op
**ไม่เขียน ledger** (กันตัวเลขทดสอบแอดมินปนสถิติรายรับ/แอฟฟิลิเอต) → มีผลทุก route อัตโนมัติ
โดยไม่แตะ decideCharge/6 routes · เพิ่มบัญชีอื่นได้ผ่าน env `UNLIMITED_CREDIT_UIDS` (คั่นจุลภาค)
· ชิปมุมขวาบนของแอดมินจะโชว์ ⭐ 999999 เครดิต (= สัญลักษณ์ว่าไม่จำกัด) · เทสต์ล็อก uid อื่นไม่ติดไปด้วย

**✅ ปรับ UX 5 จุดตามคำสั่งผู้ใช้ (10 ส.ค. 2569):**
1. **guest เป็นทางหลัก**: ปุ่ม /consent → "🔮 รับคำทำนายฟรีเลย — ไม่ต้องสมัคร" (login เป็นทางรอง
   "มีบัญชีอยู่แล้ว?") — flow เดิมอยู่ครบ แค่เปลี่ยนน้ำหนักคำ · เพิ่ม **CTA ผูกบัญชีบน /profile
   หลังการ์ดขึ้น เฉพาะ guest** (ตรวจ is_anonymous ฝั่ง browser) ชวนรับสิทธิ์เพิ่ม: ฝัน 1/เสี่ยงทาย 2/
   รางวัลแชร์ +2 → /login?next=/profile (linkIdentity เก็บข้อมูลเดิม)
2. **ลบชื่อโมเดล AI ออกจาก UI ทั้งหมด** — จุดเดียวที่โชว์คือ bubble ฝัน "ตอบโดย <provider/model>"
   ถอดแล้ว (field via ยังอยู่ใน API/log ฝั่ง server เพื่อ debug — ห้ามเอากลับมาแสดง)
3. **ปุ่มเด่นขึ้น**: S.gold หน้าแรก + ปุ่มเครื่องมือ (ขอบทองเข้ม 1.5px + fontWeight + เงาลึก)
4. **ชิปโปรไฟล์มีป้ายกำกับ**: "⭐ N เครดิต · 💬 ถามฟรี N" (เดิมมีแต่เลข ผู้ใช้ทักว่าไม่เข้าใจ)
5. **ปุ่ม "➕ เติมเครดิต" ใต้ชิปโปรไฟล์** (gradient ทอง) → /account — โชว์เฉพาะตอนล็อกอิน
verify: browser จริง (ปุ่ม consent/หน้าแรก/ชิป anon) · เทสต์ 464 · build ผ่าน · ⚠️ ชิปแบบล็อกอิน+
CTA guest บนโปรไฟล์ยังไม่ได้เห็นของจริง (ต้อง session) — โค้ดเส้นเดียวกับของเดิมที่ผ่านแล้ว

**✅ ตรวจตามคำสั่งผู้ใช้รอบ 9 ส.ค. 2569 (OG ไม่ขึ้น + FB/IG login + คำทำนายแรกพบ + คะแนน 5 ด้าน):**
1. 🔴 **ต้นเหตุ OG ไม่ขึ้นจริง: รูป OG "เย็น" หลังทุก deploy** — URL รูปมี ?hash ต่อ deploy →
   deploy ใหม่ล้างแคชทั้งชุด → ตอน MISS การ์ดใช้ 3-7 วิ (เกินที่ crawler FB/Messenger รอ) →
   พรีวิวไม่มีรูป (revalidate=86400 ช่วยเฉพาะหลังมีคนโหลดครั้งแรก) · **แก้ถาวร:**
   `/api/cron/warm-og` (gate CRON_SECRET) อุ่นรูปการ์ดทั้ง 100 + หน้าแรกรายชั่วโมง (vercel.json
...
4. **คะแนน 5 ด้าน: ตรวจคณิตทีละชั้นด้วยมือแล้วตรง 100%** (6266+ไฟขาดน้ำ: ชั้นอุปนิสัย×3 +
   ท้ายคู่×1.5 + ธาตุ+1×0.75 → 9.5/10/6.1/7.6/5.4 ภาพรวม 7.7) · ชั้นธาตุพิสูจน์ว่าทำงาน
   (มี/ไม่มีโปรไฟล์ให้คะแนนต่างกัน) · clamp 0-10 + เลข 0 นำหน้าคงไว้ ·
   **จุดหย่อนเดียวที่พบ+แก้แล้ว:** AI ไม่โชว์ตัวเลขภาพรวม → กฎ 10 บังคับ "คะแนนภาพรวมเป็นตัวเลข
   x/10 เสมอ ห้ามสรุปลอยๆ" (ยิงซ้ำ: 7.7 ขึ้นแล้ว)

**✅ วิเคราะห์คีย์เวิร์ดชุด "ฝันเห็น" ที่ผู้ใช้ส่ง + ปรับหน้าให้ครอบคำค้น 3 รูป (7 ส.ค. 2569):**
KWFinder export 25 คำ วอลุ่มรวม 22,590 — ข้อค้นพบที่ใช้ตัดสินใจได้จริง:
- **"ฝันเห็น" = 5,900 (KD 21) · "ฝัน เห็น" = 14,600 (KD 31)** เป็นคำเดียวกัน (Mangools แยกช่องว่างไทย)
  → hub เดิมยิงแค่ "ทำนายฝัน" **ปรับ title/h1 ให้ยิงทั้งสองคำ**: "ทำนายฝัน — ฝันเห็นอะไร หมายถึงอะไร"
- **คนไทยพิมพ์ 3 รูป**: ฝันเห็น / ฝันว่า / ฝันถึง → หน้ารายสัญลักษณ์ใส่ครบทั้ง 3 รูปใน description +
  บรรทัดใต้ h1 + FAQ ข้อใหม่ ("ฝันว่า X กับ ฝันถึง X ต่างกันไหม" — ตอบตามหลักจริงว่าต่างที่ฉาก
  และวันที่ฝัน ไม่ใช่คำที่ใช้เล่า)
- 🔴 **"งู" ครองคลัสเตอร์นี้** — 9 ใน 25 คำเป็นเรื่องงู (งู 2 ตัว · งู ฝัน · ฝันว่างูกัด · ดูดวงฝันเห็นงู
  · ถ้าฝันถึงงู ฯลฯ) ยืนยันว่าช่องว่าง "ไม่มีงูในฐาน" ที่เพิ่งปิดไปคือช่องว่างที่แพงที่สุดจริง
- 🔴 **สายหวย 15/25 คำ แต่วอลุ่มรวมแค่ 1,890 (8%)** — ส่วนสายความหมาย 10 คำ = 20,700 (92%)
  → **ยืนยันว่าไม่คุ้มที่จะแลกจุดยืนแบรนด์ไปกับคำหวย** ("ตีเลขจากฝัน" 920 เป็นตัวใหญ่สุดของสายนั้น)
  · เลขที่เราแสดง (เลขประจำสัญลักษณ์แบบเลี่ยงคำหวย) พอรับ traffic กลุ่มนี้ได้บางส่วนโดยไม่เสียจุดยืน
- ช่องว่างเนื้อหาที่เห็นจากคีย์เวิร์ดแต่ยังไม่มีในฐาน: **อุจจาระ/ขี้** (ขี้เต็มโถส้วม 60) และ
  **"คนที่เราชอบ"** (10) — ยังไม่เติมเอง รอชุดข้อมูลจากเจ้าของตำราตามหลักเดิม

**✅ รวม "ทำนายฝัน" เป็นโหมดเดียวในสายตาผู้ใช้ (7 ส.ค. 2569 — ผู้ใช้ทักว่าซ้ำซ้อน):**
เมนูหน้าแรกเหลือปุ่มเดียว "🌙 ทำนายฝัน" · `/dream-meaning` เป็น**ชั้นอ้างอิงของโหมดเดียวกัน**
ไม่ใช่โหมดแยก — /dream มีลิงก์ "📖 ค้นความหมายรายสัญลักษณ์" · hub มีลิงก์กลับ "โหมดทำนายฝัน"
🔴 **ห้ามยุบสองหน้าเป็น URL เดียว** — หน้าแชทต้องล็อกอิน+คำตอบต่างกันทุกครั้ง Google ไต่/จัดอันดับ
ไม่ได้เลย ถ้ายุบทิ้ง = ทิ้งช่องทาง SEO ทั้งหมดที่เพิ่งสร้าง (แยก URL แต่รวมประสบการณ์คือคำตอบ)
🐛 **บั๊กที่เจอพ่วง: `HIDE_ON` ของ LalaFloat ใช้ `startsWith`** → "/dream-meaning" ถูกนับเป็น
"/dream" ปุ่มแม่หมอเลยหายจากหน้าที่คนมาจาก Google เจอเป็นหน้าแรก — แก้เป็นเทียบ "ตรงตัวหรือ
เส้นทางย่อย" แล้ว

**✅ ตรวจไฟล์ KB ความฝัน 2 ไฟล์ที่ผู้ใช้ส่ง (7 ส.ค. 2569) — ตรงกับของในระบบ 100%:**
`Dream_Kangxi_Dictionary.xlsx` (457 แถว) เทียบกับ `data/dream_master_db.json` → **ต่างกัน 0 แถว**
· `Dream_Psychology_50themes_FINAL.xlsx` (50 ธีม) เทียบ `dream_psychology_50.json` → **ต่างกัน 0 ช่อง**
→ ยืนยัน 3 เรื่อง: (1) การนำเข้าข้อมูลเดิมไม่มีความคลาดเคลื่อน (2) **แถวซ้ำ 41 ชื่อมีอยู่ในไฟล์
ต้นฉบับเอง** ไม่ใช่บั๊กตอน import (การ dedupe ที่เพิ่มจึงถูกทาง) (3) **ช่องว่างสัตว์บกอยู่ในต้นฉบับ
จริง** — ไฟล์มีแต่สัตว์บกเชิงตำนาน (ช้างเอราวัณ ม้าบิน แมวกวัก มนุษย์หมาป่า) ไม่มีแมว/หมา/ช้าง/
เสือ/ม้า/วัว/หนู แบบธรรมดาเลย · ชื่อไฟล์ลงท้าย FINAL ยังหนุนข้อสรุป §4 ข้อ 7 (50 ธีมคือชุดสุดท้าย)

**✅ ปิดช่องว่างสัตว์บกแล้ว — ฐานสัญลักษณ์เป็น 487 (7 ส.ค. 2569, ผู้ใช้ส่ง `Dream_Kangxi_Dictionary_v3`):**
เพิ่มหมวด **"สัตว์บก / สัตว์เลี้ยงลูกด้วยนม" 30 รายการ** (สุนัข แมว วัว ควาย หมู หมูป่า ม้า ลา อูฐ แพะ
แกะ เสือ เสือดาว สิงโต ช้าง ลิง ชะนี หนู กระต่าย กวาง หมี หมาป่า จิ้งจอก แรด เม่น กระรอก พังพอน ตุ่น
ชะมด นาก) + ปรับถ้อยคำ meaning 21 แถว + 2 คอลัมน์ใหม่
- `shape_meaning` (ความหมายจากรูปลักษณ์ เช่น "ลายพาดกลอน = อำนาจที่โดดเด่น") → **แสดงเป็นหัวข้อ
  ใหม่ในหน้า SEO** เป็นเนื้อหาไม่ซ้ำใครต่อหน้า
- ✅ `lucky_number` ("เด่น 08-80 · วิ่ง 0 8") — **ผู้ใช้ตัดสิน 7 ส.ค. 2569: แสดงได้ แต่ต้องเลี่ยงคำหวย**
  → `parseSymbolNumbers()` (คู่กันทั้ง .ts/.py — golden parity) ตัดคำ "เด่น/วิ่ง" ทิ้ง เหลือตัวเลขล้วน
  แล้วแสดงในชื่อ **"เลขประจำสัญลักษณ์"** (คู่ + หลักเดี่ยว) ร่วมกับขีดคังซีในหัวข้อรหัสเชิงสัญลักษณ์
  · prompt ฝันสั่งห้ามใช้คำ "เลขเด่น/เลขวิ่ง/ศัพท์หวยใดๆ" · guardrail ปฏิเสธคำถามหวยยังทำงานเดิม
  · เทสต์ล็อก: raw string ห้ามหลุดออกจาก engine + ตรวจ HTML ที่ build แล้วว่าไม่มีศัพท์หวย
  · ยิง AI จริงยืนยัน: หัวข้อ 🔢 แสดง "เลขประจำสัญลักษณ์ บ้าน: คู่ 10, 01" พร้อม caveat ครบ
- อัปเดตพร้อมกันทุกจุด: `data/dream_master_db.json` · prompt AI-1 ทั้ง .py/.ts (457→487 — golden
  parity, regenerate fixtures) · ข้อความ UI · **migration 040 รันบน prod แล้ว** (เขียนแบบเติมอย่างเดียว
  ไม่มี DELETE — ตารางนี้เป็นฐานความรู้ของ AI-1 · รันผ่าน pooler IPv4 ตามบทเรียน §12)
- ผลลัพธ์: หน้า SEO **398 → 428 หน้า** · ทดสอบจริง "ฝันว่าแมวดำเดินมาหาที่บ้าน" → จับ แมว(ไม้)
  ได้แล้ว · "ฝันว่าขี่ม้าไปกับเสือ" → เสือ(ไม้) + ม้า(ไฟ)

🔵 **(ปิดแล้ว 7 ส.ค. 2569 — ดูบล็อกด้านบน) ช่องว่างข้อมูลที่เคยพบ — ฐาน 457 ไม่มีสัตว์บกเลย** (ไม่มี แมว หมา ช้าง เสือ ม้า วัว
หนู ลิง กระต่าย — มีแต่สัตว์น้ำ/สัตว์ปีก/แมลง/สัตว์ในตำนาน) ทั้งที่เป็นสิ่งที่คนไทยฝันและค้นหามากที่สุด
**ต้องให้ผู้ใช้ตัดสินก่อนเติม** (ห้ามแต่งเอง): ขอชุดข้อมูลจากเจ้าของตำรา · หรือใช้ AI-1 ค้นคว้า
(~฿7.46/สัญลักษณ์ แคชถาวรใช้ได้ทั้งระบบ+หน้า SEO) · หรือข้ามไปก่อน

**✅ หน่วยเครดิต ×10 ทั้งระบบ (7 ส.ค. 2569 — ผู้ใช้สั่ง):** แพ็ก ฿29→**90** · ฿59→**210** ·
฿129→**510** เครดิต และอัตราหักคูณสิบพร้อมกัน (แชท 10 · ฝัน/เสี่ยงทาย 20 · โลโก้ 10/70 · ฉลาก 70 ·
vision 10) · เพดานเติมมือแอดมิน 500→5,000 · **อัตราส่วนเงิน:ของ ไม่เปลี่ยนเลย กำไรทุกรายการเท่าเดิม**
(ค่าเครดิตลดจาก ฿2.53 → ฿0.253 ที่ floor) — เหตุผลคือจิตวิทยาราคา "จ่าย ฿29 ได้ 90 เครดิต" ·
เทสต์กำไร ≥500% ยังล็อกเหมือนเดิมทุกแพ็ก (463 เทสต์ผ่าน)

**✅ NamePower นับสระ/วรรณยุกต์แล้วทุกจุด (6 ส.ค. 2569 — ผู้ใช้ตัดสิน "นับสระทุกจุด"):**
- **หลักฐานชี้ขาด:** ผู้ใช้ส่งตารางทางการ `Calculation_Constants.xlsx - Name_Numerology`
  (มีค่าสระ/วรรณยุกต์: ุ า ำ ้=1 · ู ่=2 · ิ=3 · โ เ แ=4 · ใ ั=6 · ไ ์=9) + สเปก ง.2 เขียนชัด
  "แปลงตัวอักษร**ทั้งหมด**" — พยัญชนะ+อังกฤษทั้ง 9 กลุ่มตรงโค้ดเดิม 100% (ยืนยัน ส=7, อ=6 อีกชั้น)
- **จุดแก้:** `officialCharValues()` ใน card-id.ts (จุดเดียว) → namePower + naming.ts
  (nameElement/nameComposition ผ่าน nameGroups) + naming_branding_engine.py (official_char_groups
  — golden parity, regenerate fixtures แล้ว) · reverseGenerateCandidates คงใช้พยัญชนะ (จับต้นพยางค์)
- **กฎแยกบริบท "อ" (ตารางให้ 2 ค่า):** อ ต้นคำ/หลังอักขระไม่ใช่ไทย/หลังสระหน้า (เ แ โ ใ ไ)/
  มีรูปสระ-วรรณยุกต์เกาะตามหลัง = พยัญชนะ 6 · ตามหลังพยัญชนะ = สระออ 4 · หลัง ื = ข้าม
  (เทสต์ล็อก: อร=10 เอก=11 อ่าน=14 สมอ=16 มือ=5 สมชาย=23)
- ✅ **7 อักขระที่ตารางเดิมไม่มี เจ้าของตำราให้ค่าครบแล้ว 7 ส.ค. 2569**: ี ึ = 3 (ตระกูลสระอิ) ·
  ื = 2 (ตระกูลสระเอือ) · ะ = 1 (ตระกูลสระอา) · ็ = 6 (ตระกูลไม้หันอากาศ) · ๊ = 3 · ๋ = 4
  (ลำดับวรรณยุกต์ เอก=2 โท=1 ตรี=3 จัตวา=4) — ใส่ครบทั้ง .ts/.py/fixture generator แล้ว
  **สระประสม (เ-ือ/เ-ีย) นับรายอักขระ** ตามที่เจ้าของตำรารับรองว่าแยกรูปได้ (อ ในรูป ือ ข้าม
  เพราะนับที่ ื แล้ว) · เทสต์ล็อกเพิ่ม: ดี=4 ถึง=6 จะ=7 ก็=7 เก๊=8 ก๋วย=19 เสือ=13 มือ=7
  ⏳ เหลือขอยืนยันกฎแยกบริบท "อ" ข้อเดียวในเอกสารรอบสอง
- 🔴 **การ์ดสูตร A เปลี่ยนใบสำหรับชื่อที่มีสระ** (ผู้ใช้รับทราบ — ช่วงเปิดตัวผู้ใช้น้อย เปลี่ยนตอนนี้
  เจ็บน้อยสุด) · **gen_cardid_fixtures.mjs เลิก parity กับ intake_form.html สำหรับ namePower/
  computeCardId โดยเจตนา** — ใช้ implementation อิสระของตารางทางการ cross-check แทน
  (digitSum/reduceTo99 ยังเทียบ prototype) · ผลพวง: สมชาย เลขศาสตร์ 22→23 · ชื่อที่มีสระบน/
  วรรณยุกต์เปลี่ยนมากกว่า (วุฒิ์ธิระ ครุฑขุนทด: การ์ด 3→4 · กชกร บุญเรือง: 91→93) · องค์ประกอบธาตุ
  ทอง 50/ดิน 25/ไม้ 25 → ทอง 40/ดิน 20/ไม้ 40 (า=กลุ่ม 1=ไม้) — เทสต์ปรับแล้ว 454 ผ่าน

**✅ เจ้าของตำราตอบคำถาม 6 ข้อแล้ว (6 ส.ค. 2569 — ปิดหนี้ความถูกต้องชุดใหญ่):**
- **ข้อ 1 "อ" = กลุ่ม 6 ✅** (ระบบถูกแล้ว — ธง §4 ข้อ 6 ปิดสมบูรณ์) · **ข้อ 2 ส = กลุ่ม 7 ตามตาราง ✅**
  (ตัวอย่าง "สมชาย" ในตำราพิมพ์ผิด — โค้ดเราถูกอยู่แล้ว) ✅ **ประเด็นนับสระปิดแล้ว**
  — ผู้ใช้ส่งตาราง Calculation_Constants + ตัดสิน "นับสระทุกจุด" (ดูบล็อก ✅ NamePower ด้านบน)
- **ข้อ 3 สูตรเบอร์โทรทางการ 🎉** — `lib/engine/phone-official.ts` (3 เทสต์): ตัด 2 ตัวหน้า →
  รายคู่ 4 คู่ (ความหมายจากตาราง 00-99) + เลขกำลังรวม · น้ำหนัก **ผลรวม 40% / คู่ท้าย 25% /
  คู่รอง 15% / คู่อื่น 10%×2** · fn ที่ 21 `myPhoneReading` (คู่กับ myNumberAspects · เลิกใช้
  analyzePhoneNumber ในแชท — คงไว้เพื่อ golden parity เท่านั้น) · **ธง §5 "สูตรเบอร์ไม่ verify"
  ปิดแล้ว** · จุดตีความที่จดไว้ในหัวไฟล์: "ผลรวมทั้งหมด" = ทุกหลักทั้งเบอร์ · คะแนนรายส่วนยังใช้
  กลไก numberAspects (สูตรเสริม — โครง+น้ำหนักคือของทางการ)
- **ข้อ 5 คู่มิตร-คู่ศัตรู** → ใช้หลักธาตุแทน ✅ (ตรงระบบ — ตัด V.8 ทิ้งถาวร ไม่ต้อง verify แล้ว)
- **ข้อ 6 เลข 3 หลัก** → 14 แถวพอ นอกนั้นตีความรายหลักปกติ ✅ (fallback ปัจจุบันได้รับยืนยัน —
  เปลี่ยนสถานะจาก "ออกแบบเอง" เป็น "เจ้าของสูตรยืนยัน" · caveat ปรับแล้ว)
- **ข้อ 4 ตาราง 9 กลุ่ม→ธาตุ** → เจ้าของตำราขอข้อมูล/ทางเลือกเพิ่ม ⏳ **งานรอบหน้า: เตรียม
  เอกสารทางเลือก** (เช่น ตารางปัจจุบัน vs จัดตามเสียงอักษร vs จัดตามธาตุนักษัตร) ส่งรอบสอง
- verify AI จริง: "เบอร์ 081-234-5678 ดีไหม" → myPhoneReading+myNumberAspects การ์ดผลรวม 44
  "จอมคนผู้สร้างเมือง" + รายคู่ + คะแนน 5 ด้าน · เทสต์ 453 · คำตอบบันทึกท้าย docs/คำถามถึงเจ้าของตำรา

**✅ ฝันฟรี 1 ครั้ง + "รหัสพลังงานเชิงสัญลักษณ์" ในคำทำนายฝัน (6 ส.ค. 2569 — ผู้ใช้ตัดสิน):**
- **สิทธิ์ฝัน 2→1 ครั้ง/บัญชี** (`freeLimitForLogic` override {4:1} — เสี่ยงทาย/อื่นๆ คง 2 ·
  คำถามแชท 3 ข้อคงเดิม) — UI/ข้อความทุกจุดปรับตาม · เหตุผล: ฝันคือ action ต้นทุนสูงสุด (AI-1 ฿7.46)
- **นโยบายเลขในฝัน (ผู้ใช้ชี้ขาด): "อย่ามองเป็นใบ้หวย ให้มองเป็นการคำนวณ"** →
  `lib/engine/dream-energy.ts` (3 เทสต์) รหัสพลังงาน ฿0 ล้วน: **ขีดอักษรคังซีต่อสัญลักษณ์**
  (มีใน dream DB อยู่แล้ว — ไม่ต้องรอ Kaekled!) + เลขดาวประจำวันฝัน (อาทิตย์ 1...เสาร์ 7) +
  เลขประจำธาตุ (reverse ตาราง §5.4) + สีนำโชคช่วงนั้น (ELEMENT_TO_COLORS ของธาตุวัน) ·
  DREAM_ENERGY_CAVEAT "ไม่ใช่คำแนะนำการเสี่ยงโชค" บังคับ + prompt ห้ามชวนซื้อหวย ·
  guardrail หวย (ถามตรง) ยังทำงานเดิม
- verify AI จริง: ฝันงูเห่า+บ้าน → โครงครบ สัญลักษณ์+จิตวิทยา+ธาตุวัน+รหัสพลังงาน · เทสต์ 450
- ⚠️ พบระหว่างทดสอบ: "งูเห่า" ไม่แมตช์ DB "งู / พญานาค" (ขอบคำ — ICU มอง "งูเห่า" เป็นคำเดียว)
  = ช่องว่าง matching ที่รู้ตัวแล้ว ควรเก็บใน phase ฝัน (เพิ่ม alias/split คำประสม)

**✅ ยกเครื่องคำทำนายสายเลขตามตัวอย่าง PDF ระบบเก่า + โหมดแนะนำเลขใหม่ (6 ส.ค. 2569 — ผู้ใช้แนบ
PDF 4 ไฟล์ "ควรนำมาปรับใช้"):**
- **engine เพิ่ม 2 ชั้นจากตาราง 00-99 จริง:** `ผลรวมเลข→การ์ดผลรวม` (จง 6366: sum 21 = ดาวเด่น)
  + `letters` พลังอักษรป้าย (จง = จ6+ง2 = 8 → รวม 29 = ผู้ศรัทธาที่เมตตา) — **ตรง PDF ตัวอย่างเป๊ะ
  ทั้งคู่** (namePower + lookup2digit เดิมที่ผ่าน golden test)
- **fn ที่ 20 `myNumberSuggest {prefix, letters?, count?}`** — โหมด "กำลังจะไปขอเลขใหม่ เลขไหนดี"
  (เคส PDF D): สแกนเลขท้าย 00-99 ต่อท้าย prefix → จัดอันดับด้วยภาพรวม 5 ด้าน deterministic
- **narrator เพิ่ม:** เล่า "บุคลิกของเลข" จากการ์ดก่อนลงคะแนน (สไตล์โหรอ่านการ์ด) · เทียบหลายเลข →
  ตารางมิติ + "แม่หมอแนะนำ" พร้อมเหตุผล+บริบทผู้ใช้ (**ห้ามใช้คำ "ฟันธง"** — PDF เดิมใช้ แต่ขัดกฎเรา)
- UI: หน้าแรกสลับปุ่ม ทำนายฝัน→บนสุด, ทะเบียนรถ→ถัดมา (ผู้ใช้สั่ง) · /lucky-number แก้สีตัวหนังสือ
  ผลทำนายเป็นเข้ม #1d1812 (เดิมจางอ่านไม่ได้ — screenshot ผู้ใช้) + โชว์การ์ดผลรวม
- **verify AI จริง 2 เคสตรงจาก PDF:** "จง 6366 เทียบ 6066/6266" → 3 calls พร้อม letters บุคลิกการ์ด
  ครบ · "ขอทะเบียนขึ้นต้น 60" → myNumberSuggest จัดอันดับ 6099 (9/10) · เทสต์ 446
- 💡 PDF C (โลโก้ scorecard 5 ด้าน) — แนวเดียวกับ /logo ที่มี wuXingScore อยู่แล้ว ยังไม่ได้ทำ
  scorecard เต็ม (จดไว้เป็นงานเสริมของ /logo รอบหน้า) · เฟส 2 ทำนายฝัน 457 หน้า ยังคิวถัดไป

**✅ SEO เฟส 1 (6 ส.ค. 2569 — แผน 3 เฟสที่ตกลงกับผู้ใช้ · เฟส 2 = อาณาจักรทำนายฝัน รอบถัดไป):**
- **`/lucky-number`** — landing "ดูดวงทะเบียนรถ เบอร์โทร บ้านเลขที่" (คีย์เวิร์ดเป้า: ดูดวงทะเบียนรถ
  15.9k/เดือน **KD 2** + เบอร์มือถือ/โทรศัพท์ ~19.6k จากข้อมูล Mangools ของผู้ใช้) — จุดต่างจาก
  คู่แข่ง: **เครื่องคิดเลขจริงในหน้า** (numberAspects รันฝั่ง client ฿0 ไม่ต้องล็อกอิน) + เนื้อหา SEO
  static + CTA เปิดการ์ด · ไม่มีโปรไฟล์ = คะแนนจากตัวเลขล้วน (บอกตรงๆ + ชวนล็อกอินดูชั้นธาตุ)
- **`/ai`** — content "ดูดวง AI แม่นไหม" ยึดคลัสเตอร์คีย์เวิร์ดเกิดใหม่ (ดูดวง ai/แม่นไหม/ไหนแม่นสุด —
  โผล่ autocomplete แต่ volume ยังไม่ถูกบันทึก = ชิงก่อนใคร) — เนื้อหา = จุดยืน §16 ตรงๆ:
  AI เรียบเรียงจากผลคำนวณ ไม่ใช่เดา + ข้อจำกัดที่บอกตรงๆ
- sitemap เพิ่ม 2 หน้า · TOOLS หน้าแรกเพิ่ม 🔢 · title /profile → "ดูดวงแม่นๆ จากวันเดือนปีเกิด..."
  (เป้า 3k/เดือน KD 10) · verify browser: คำนวณจริง 5 ด้าน+caveat+CTA ครบ ทั้งสองหน้าเรนเดอร์
- **เฟส 2 ที่ตกลงไว้ (รอบถัดไป):** เปิดใช้ `Unified_Kaekled_DB` (ขีดคังซี+แก้เคล็ด 3 ศาสตร์+มนตรา —
  ไฟล์อยู่ใน data/raw-uploads ยังไม่เคยเปิด) → (ก) ยกเครื่องคำตอบ /dream เป็นโครง 5 ส่วน
  "เล่าฝันเป็นฉาก" ตามตัวอย่างที่ผู้ใช้ให้ (⚠️ ตัด CTA "ขอเลขชุด" — ชน guardrail หวย · "รหัสพลังงาน"
  คงได้ในฐานะเลขเชิงสัญลักษณ์) (ข) หน้า SEO ทำนายฝัน 457 สัญลักษณ์ + hub (เป้า "ทำนายฝัน" 274k
  KD 23 — SERP อ่อน: อันดับ 2 DA 8) · ผู้ใช้จะ export Related keywords "ทำนายฝัน" มาให้จัดลำดับ
...
- planner: เบอร์โทร → เรียก **myNumberAspects คู่กับ analyzePhoneNumber** (คะแนน 5 ด้าน + ความหมาย
  ...
  myNumberAspects/numberAspects รับสตริงหลักล้วน ≤10 ตัวได้แล้ว (มีเทสต์ล็อก "0812345678")
- verify AI จริง: "เบอร์ 081-234-5678 ดีไหม" → planner ส่ง num:"0812345678" (คง 0) + คะแนนครบ 5 ด้าน
  ตรง engine เป๊ะ · เทสต์ 444
...
1. **AuthStatus มุมขวาบน**: ปรับ 2 รอบ — พื้นทองทึบยังกลืน → **พื้นดำอมน้ำตาล #221c14 + ตัวทอง**
   (ผู้ใช้ตัดสิน — แบบเดียวกับปุ่มแมวลอยที่อ่านชัด) · ไอคอน 🐾 → 👤 · ตรวจ preview แล้ว
