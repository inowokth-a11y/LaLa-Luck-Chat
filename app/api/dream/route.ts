...
//   2. engine matching จากฐาน 487 สัญลักษณ์ + 50 ธีม (deterministic)
...
import { kaekledForSymbols } from "@/lib/engine/kaekled";
...
กฎเหล็ก (ห้ามฝ่าฝืน):
1. ใช้ได้เฉพาะข้อมูลใน <ผลการวิเคราะห์> ที่ให้มาเท่านั้น — ห้ามแต่งสัญลักษณ์ ธาตุ ความหมาย
   วิธีแก้เคล็ด หรือบทสวดขึ้นเอง (ถ้าไม่มีให้มา = ไม่ต้องพูดถึง)
1.1 🔴 สิ่งที่ผู้ใช้เล่าแต่ **ไม่อยู่ในรายการสัญลักษณ์ที่พบ** (เช่น สัตว์หรือของบางอย่างในฉาก)
   — พูดถึงได้เฉพาะในฐานะ "รายละเอียดของฉาก" เท่านั้น **ห้ามให้ธาตุ ขีดอักษรคังซี หรือ
   ความหมายเชิงสัญลักษณ์กับมันเด็ดขาด** ถ้าผู้ใช้ถามถึงสิ่งนั้นตรงๆ ให้บอกตามตรงว่ายังไม่มี
   ข้อมูลของสัญลักษณ์นี้ในฐาน — **ห้ามอธิบายว่าสิ่งนั้น "สื่อถึง" อะไร แม้จะอ้างว่าเป็นความรู้
   ทางจิตวิทยาทั่วไป** (จุดขายของระบบคือทุกความหมายมาจากฐานข้อมูลจริง ไม่ใช่ความเห็นของ AI)
...
5. **ผู้ใช้เล่าฝันมาเป็นฉาก — ให้อ่านทั้งฉาก** ไม่ใช่ตอบแค่คำเดียว: ใครทำอะไร เกิดที่ไหน
   อะไรเปลี่ยนสถานะระหว่างฉาก (เช่น ของหลุดออกมา ของขาด คนเดินจากห้องหนึ่งไปอีกห้อง)
   แล้วเชื่อมรายละเอียดเหล่านั้นกับสัญลักษณ์ที่ระบบจับได้
5.1 🔴 **ยึดแกนเรื่อง** — ฟิลด์ <แกนเรื่อง> คือสิ่งที่ผู้ใช้เล่าเป็นตัวเอกของฝัน (สัญลักษณ์ที่เรียงมาก่อน
   = สำคัญกว่า) ความหมายหลัก บทสรุป และคำถามปิดท้าย **ต้องวนอยู่กับแกนเรื่องนี้** · สัญลักษณ์ที่เป็น
   กริยาประกอบ (เดิน ยิ้ม ให้ วิ่ง กิน ไม่ทัน ฯลฯ) เอ่ยได้แค่สั้นๆ ในฐานะจังหวะของฉาก **ห้ามยกความหมาย
   ของกริยาประกอบขึ้นเป็นข้อสรุปหลักหรือคำถามปิดท้าย** (เช่น ฝันถึงแม่ที่จากไป ห้ามสรุปว่า "กลัวทำอะไร
   ไม่ทันเวลา" เพราะคำว่า "ไม่ทัน") · ถ้าผู้ใช้บอกสถานะของตัวเอก (เสียไปแล้ว/แฟนเก่า/เพื่อนเก่า) ให้เล่าตาม
   สถานะนั้น ห้ามสลับเป็นคนปัจจุบัน
6. ส่วน "รหัสพลังงาน" ให้เล่าเป็น **เลขจากการคำนวณ** (ขีดอักษรคังซี เลขที่ตำราผูกกับสัญลักษณ์
   เลขดาวประจำวันฝัน เลขประจำธาตุ) — เรียกว่า "เลขประจำสัญลักษณ์" **ห้ามใช้คำว่า เลขเด่น เลขวิ่ง
   เลขให้ หรือศัพท์หวยใดๆ** พร้อมสีนำโชคของช่วงเวลา — **ห้ามชวนนำเลขไปเสี่ยงโชค/ซื้อหวย ห้ามเสนอ
   "ขอเลขชุด"** และต้องคงหมายเหตุที่ให้มาไว้ในคำตอบ
7. ส่วน "แนวทางคลี่คลาย" — ถ้าที่มาเป็น "ตำราแก้เคล็ด" ให้เล่าตามที่ให้มาได้เลย
   ถ้าที่มาเป็น "หลักธาตุ" ต้องบอกตรงๆ ว่าเป็นแนวทางตามหลักธาตุ ไม่ใช่พิธีที่ระบุในตำรา
   และต้องคงหมายเหตุเรื่องความเชื่อไว้ · เสนอเป็นทางเลือก ไม่ใช่คำสั่ง
8. ความยาวรวมไม่เกิน ~6 ย่อหน้าสั้น จัดหัวข้อให้อ่านง่ายบนมือถือ

โครงคำตอบ (ใช้หัวข้อตามนี้ ข้ามหัวข้อที่ไม่มีข้อมูลได้):
🗝️ ความหมายหลัก — เริ่มจากแกนเรื่องก่อนเสมอ (ธาตุ + ขีดอักษรคังซี + ความหมายตามฐานข้อมูล)
   แล้วค่อยสัญลักษณ์รองในฉาก — ไม่ต้องไล่ทุกตัวเท่ากัน
⏳ ปัจจัยแวดล้อม — วันที่ฝันและธาตุประจำวัน + จังหวะของฉาก (อะไรเปลี่ยนไประหว่างเรื่อง)
   + เชื่อมกับสิ่งที่ผู้ใช้เคยเล่าไว้ ถ้ามีในความจำ
🔢 รหัสพลังงาน — เลขจากการคำนวณ + สีนำโชคช่วงนี้ (พร้อมหมายเหตุ)
🕯️ แนวทางคลี่คลาย — ธาตุเชื่อมและสิ่งที่ทำได้ (ตามที่ให้มาเท่านั้น)
🔮 บทสรุปเชิงหลักการ — สรุปสั้น ไม่ฟันธง โยงกลับแกนเรื่อง + ปิดท้ายด้วยคำถามชวนคุยต่อ 1 ข้อ
   ที่เกี่ยวกับแกนเรื่อง`;
...
หาสัญลักษณ์หลักในความฝันนี้ที่ยังไม่มีในฐานข้อมูล 487 รายการ แล้วตัดสินธาตุจากความหมายเชิงสัญลักษณ์
...
\`\`\``,
    ...
    // แนวทางแก้เคล็ดจากตำรา Unified_Kaekled_DB (เฟส 2) — ฿0 · ตำรามาก่อน แล้วค่อยหลักธาตุ
    const kaekled = kaekledForSymbols(result.symbol_matches?.map((m) => ({ dream_object: m.object, element: m.element })) ?? []);

    // แกนเรื่อง vs สัญลักษณ์รอง (10 ก.ย. 2569) — ติดป้ายบทบาทในตัวข้อมูล + ย้ำในข้อความสั่งงาน
    // (ทดสอบแล้ว: ใส่กฎไว้ใน system อย่างเดียว Flash ยังหยิบความหมายของ "ไม่ทัน" มาปิดท้าย)
    const coreNames = result.core_symbols ?? [];
    const secondaryNames = (result.symbol_matches ?? []).map((m) => m.object).filter((o) => !coreNames.includes(o));
    const focusLine = coreNames.length
      ? `\n\n🔴 แกนเรื่องของฝันนี้: ${coreNames.join(", ")} — ความหมายหลัก บทสรุป และคำถามปิดท้ายต้องยึดแกนเรื่องนี้เท่านั้น` +
        (secondaryNames.length
          ? ` · สัญลักษณ์รอง (${secondaryNames.join(", ")}) เป็นเพียงรายละเอียดของฉาก เอ่ยสั้นๆ ได้ แต่ห้ามยกความหมายของมันขึ้นเป็นข้อสรุปหรือคำถามปิดท้าย`
          : "")
      : "";
    ...
    const context = JSON.stringify(
      {
        ความฝัน: dreamText,
        แกนเรื่อง: coreNames.length ? coreNames : null,
        สัญลักษณ์ที่พบในฐานข้อมูล: (result.symbol_matches ?? []).map((m) => ({
          บทบาท: coreNames.includes(m.object) ? "แกนเรื่อง" : "รอง (รายละเอียดฉาก)",
          ...m,
        })),
        ...
        แนวทางคลี่คลาย: kaekled.รายการ.length ? kaekled : null,
...
function renderTemplate(
  ...
  const kaekled = kaekledForSymbols((r.symbol_matches ?? []).map((m) => ({ dream_object: m.object, element: m.element })));
  const lines: string[] = [];
  ...
  if (kaekled.รายการ.length) {
    lines.push("", "แนวทางคลี่คลาย:");
    for (const g of kaekled.รายการ) {
      lines.push(`• ${g.สัญลักษณ์} (ธาตุเชื่อม: ${g.ธาตุเชื่อม}) — ${g.แนวทางไทย ?? g.กิจกรรมเปลี่ยนพลัง}`);
    }
    lines.push(kaekled.หมายเหตุ);
  }
